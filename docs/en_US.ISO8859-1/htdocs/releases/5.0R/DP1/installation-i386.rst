...
.. raw:: html
    ...
    FreeBSD/i386 5.0-DP1, with particular emphasis given to obtaining a
    FreeBSD distribution. Some notes on troubleshooting and
...
obtaining the FreeBSD 5.0-DP1 distribution and to beginning the
...
hard drive space for the most minimal installation. See below for ways
...
-  If your system supports bootable CDROM media (usually an option which
   ...
   the \`\`El Torito'' bootable CD standard. Simply put the installation
   CD in your CDROM drive and boot the system to begin installation.
...
-  If you would be able to FTP install FreeBSD directly from the CDROM
   ...
   `vipw(8) <http://www.FreeBSD.org/cgi/man.cgi?query=vipw&sektion=8&manpath=FreeBSD+5.0-current>`__
   command):
...
`doscmd(1) <http://www.FreeBSD.org/cgi/man.cgi?query=doscmd&sektion=1&manpath=FreeBSD+5.0-current>`__
...
``newsyslog.conf``, ``nsmb.conf``, ``nsswitch.conf``, ``pam.conf``,
...
checking, repairing and examining filesystems and their contents. Some
...
upgrade the BIOS (see Intel's web site for details).
