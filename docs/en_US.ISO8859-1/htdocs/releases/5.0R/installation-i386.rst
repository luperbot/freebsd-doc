==================================================
FreeBSD/i386 5.0-RELEASE Installation Instructions
==================================================
...
.. raw:: html
    ...
    FreeBSD/i386 5.0-RELEASE, with particular emphasis given to
...
obtaining the FreeBSD 5.0-RELEASE distribution and to beginning the
...
and ``mfsroot.flp`` images (for 1.44MB floppies). Depending on your
hardware, you may also need to make the third ``drivers.flp`` image to
provide necessary device drivers.

Getting these images over the network is easy. Simply fetch the
``release``\ ``/floppies/kern.flp``,
``release``\ ``/floppies/mfsroot.flp``, and
``release``\ ``/floppies/drivers.flp`` files from ftp://ftp.FreeBSD.org/
...
-  If you would be able to FTP install FreeBSD directly from the CDROM
   ...
   `vipw(8) <http://www.FreeBSD.org/cgi/man.cgi?query=vipw&sektion=8&manpath=FreeBSD+5.0-RELEASE>`__
   command):
...
`tar(1) <http://www.FreeBSD.org/cgi/man.cgi?query=tar&sektion=1&manpath=FreeBSD+5.0-RELEASE>`__
...
`doscmd(1) <http://www.FreeBSD.org/cgi/man.cgi?query=doscmd&sektion=1&manpath=FreeBSD+5.0-RELEASE>`__
...
#. The ``packages`` and ``ports`` directories contain the FreeBSD
   ...
   ``packages``/ to the
   `pkg\_add(1) <http://www.FreeBSD.org/cgi/man.cgi?query=pkg_add&sektion=1&manpath=FreeBSD+5.0-RELEASE>`__
   command.
...
`mtree(8) <http://www.FreeBSD.org/cgi/man.cgi?query=mtree&sektion=8&manpath=FreeBSD+5.0-RELEASE>`__
...
.. raw:: html
    ...
    **Warning:** Binary upgrades to FreeBSD 5.0-RELEASE from FreeBSD
    ...
    upgrading from relatively recent FreeBSD 5-CURRENT snapshots.
...
``master.passwd``, ``motd``, ``namedb``, ``networks``,
...
probed. <#Q4.2.1.>`__
4.2.2. `My legacy ISA device used to be recognized in previous versions
of FreeBSD, but now it's not. What happened? <#Q4.2.2.>`__
4.2.3. `I go to boot from the hard disk for the first time after
installing FreeBSD, the kernel loads and probes my hardware, but stops
with messages like: <#Q4.2.3.>`__
...
the boot menu each time but the boot won't go any further. <#Q4.2.4.>`__
4.2.5. `The mcd(4) driver keeps thinking that it has found a device and
this stops my Intel EtherExpress card from working. <#Q4.2.5.>`__
4.2.6. `The system finds my ed(4) network card, but I keep getting
device timeout errors. <#Q4.2.6.>`__
4.2.7. `I booted the install floppy on my IBM ThinkPad (tm) laptop, and
the keyboard is all messed up. <#Q4.2.7.>`__
4.2.8. `My system can not find my Intel EtherExpress 16
card. <#Q4.2.8.>`__
4.2.9. `When installing on an EISA HP Netserver, my on-board AIC-7xxx
SCSI controller isn't detected. <#Q4.2.9.>`__
...
now. <#Q4.2.10.>`__
4.2.11. `I have this CMD640 IDE controller that is said to be
broken. <#Q4.2.11.>`__
4.2.12. `On a Compaq Aero notebook, I get the message \`\`No floppy
devices found! Please check ...'' when trying to install from
floppy. <#Q4.2.12.>`__
4.2.13. `When installing on an Dell Poweredge XE, Dell proprietary RAID
controller DSA (Dell SCSI Array) isn't recognized. <#Q4.2.13.>`__
...
doesn't connect to the network. <#Q4.2.14.>`__
4.2.15. `When I configure the network during installation on an IBM
Netfinity 3500, the system freezes. <#Q4.2.15.>`__
...
\ **4.2.5.** The mcd(4) driver keeps thinking that it has found a device
...
\ **4.2.7.** I booted the install floppy on my IBM ThinkPad (tm) laptop,
...
\ **4.2.9.** When installing on an EISA HP Netserver, my on-board
...
\ **4.2.10.** I have a Panasonic AL-N1 or Rios Chandler Pentium machine
...
\ **4.2.13.** When installing on an Dell Poweredge XE, Dell proprietary
...
\ **4.2.14.** I have an IBM EtherJet PCI card, it is detected by the
