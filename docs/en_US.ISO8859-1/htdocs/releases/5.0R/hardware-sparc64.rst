==========================================
FreeBSD/sparc64 5.0-RELEASE Hardware Notes
==========================================
...
2.1 `Overview <#AEN19>`__
2.2 `In general, what do you need to run FreeBSD on a
sparc64 <#AEN30>`__
2.3 `System-specific information <#AEN57>`__
2.3.1 `Ultra 1 <#AEN61>`__
2.3.2 `Ultra 2 <#AEN65>`__
2.3.3 `Ultra 5/10 <#AEN82>`__
2.3.4 `Ultra 60 <#AEN110>`__
2.3.5 `Blade 100 <#AEN134>`__
2.4 `Supported Hardware Overview <#AEN173>`__
3 `Supported Devices <#SUPPORT-DEV>`__
3.1 `PCI Hardware <#AEN202>`__
3.2 `SBus Hardware <#AEN240>`__
3.3 `ISA Hardware <#AEN250>`__
3.4 `EBus Hardware <#AEN260>`__
3.5 `Unsupported Hardware <#AEN273>`__
4 `Acknowledgments <#ACKNOWLEDGEMENTS>`__
...
5.0-RELEASE on the UltraSPARC hardware platform (also referred to as
FreeBSD/sparc64 5.0-RELEASE). It lists devices known to work on this
...
The GENERIC kernel configuration file in ``/sys/sparc64/conf/GENERIC``
may contain additional information about supported devices.
...
-  `sym(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sym&sektion=4&manpath=FreeBSD+5.0-RELEASE>`__-based
...
-  `sio(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sio&sektion=4&manpath=FreeBSD+5.0-RELEASE>`__
   supported serial ports for ttya and ttyb
...
driver framework. *(not yet committed - in Perforce tree)*
...
cards should be supported soon. Be aware that SCSI cards must contain
Sun FCODE in order to use them as a boot device from OpenBoot.
...
$FreeBSD:
src/release/doc/en\_US.ISO8859-1/hardware/sparc64/dev-sparc64.sgml,v
...
-  `ata(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ata&sektion=4&manpath=FreeBSD+5.0-RELEASE>`__,
   ``atadisk``, ``atapicd`` (but not atapifd, atapist)
...
-  Siemens sab82532 serial ports (Ultra 5/10; the keyboard/mouse ports
   are also RS232 ones and supported by the sio driver though; a driver,
   'se', is being worked on).
...
obtained from `Sun <http://www.sun.com>`__.
