...
| $FreeBSD: src/release/doc/en\_US.ISO8859-1/readme/article.sgml,v
  1.19.2.2 2003/01/14 18:12:51 roam Exp $
...
.. raw:: html
    ...
    This document gives a brief introduction to FreeBSD 5.0-RELEASE. It
...
This distribution is a release of FreeBSD 5.0-RELEASE, the latest point
...
a piece of software, with a single command. Over 7,800 ports, from
...
the FreeBSD-CURRENT mailing list ``<freebsd-current@FreeBSD.org>``, in
...
`send-pr(1) <http://www.FreeBSD.org/cgi/man.cgi?query=send-pr&sektion=1&manpath=FreeBSD+5.0-RELEASE>`__
...
-  ``EARLY.TXT``: A guide for early adopters of FreeBSD 5.0-RELEASE.
...
-  ``RELNOTES.TXT``: The release notes, showing what's new and different
   in FreeBSD 5.0-RELEASE compared to the previous release (FreeBSD
   4.0-RELEASE).
...
(currently the i386 and alpha), these documents are generally available
...
`tuning(7) <http://www.FreeBSD.org/cgi/man.cgi?query=tuning&sektion=7&manpath=FreeBSD+5.0-RELEASE>`__
(a guide to performance tuning),
`security(7) <http://www.FreeBSD.org/cgi/man.cgi?query=security&sektion=7&manpath=FreeBSD+5.0-RELEASE>`__
(an introduction to FreeBSD security), and
`style(9) <http://www.FreeBSD.org/cgi/man.cgi?query=style&sektion=9&manpath=FreeBSD+5.0-RELEASE>`__
