...
.. raw:: html
    ...
    FreeBSD/i386 5.4-RELEASE, with particular emphasis given to
...
obtaining the FreeBSD 5.4-RELEASE distribution and to beginning the
...
`dd(1) <http://www.FreeBSD.org/cgi/man.cgi?query=dd&sektion=1&manpath=FreeBSD+5.4-RELEASE>`__
command in UNIX.
...
-  If you would be able to FTP install FreeBSD directly from the CDROM
   ...
   `vipw(8) <http://www.FreeBSD.org/cgi/man.cgi?query=vipw&sektion=8&manpath=FreeBSD+5.4-RELEASE>`__
   command):
...
`disklabel(8) <http://www.FreeBSD.org/cgi/man.cgi?query=disklabel&sektion=8&manpath=FreeBSD+5.4-RELEASE>`__
and
...
`doscmd(1) <http://www.FreeBSD.org/cgi/man.cgi?query=doscmd&sektion=1&manpath=FreeBSD+5.4-RELEASE>`__
...
#. The ``packages`` and ``ports`` directories contain the FreeBSD
   ...
   ``packages``/ to the
   `pkg\_add(1) <http://www.FreeBSD.org/cgi/man.cgi?query=pkg_add&sektion=1&manpath=FreeBSD+5.4-RELEASE>`__
   command.
...
`mtree(8) <http://www.FreeBSD.org/cgi/man.cgi?query=mtree&sektion=8&manpath=FreeBSD+5.4-RELEASE>`__
...
.. raw:: html
    ...
    **Warning:** Binary upgrades to FreeBSD 5.4-RELEASE from FreeBSD
    ...
    upgrading from relatively recent FreeBSD 5-STABLE snapshots.
...
probed. <#Q4.2.1.>`__
4.2.2. `My system uses the nVidia nForce3 Pro-150 chipset and I get
hangs at the end of boot or suddenly starts losing
...
ACPI driver and within system motherboards and BIOS. The use of ACPI can
be disabled by setting the \`\`hint.acpi.0.disabled'' hint in the third
stage boot loader:
