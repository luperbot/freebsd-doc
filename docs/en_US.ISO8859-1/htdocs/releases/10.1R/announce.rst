...
stable/10 branch, which improves on the stability of
FreeBSD?10.0-RELEASE and introduces some new features.
...
-  Support for SMP was added to the ``armv6`` kernels and enabled by
   default in the configuration files for all platforms that contain
   multi-core CPUs.
...
-  The sshd(8) rc.d(8) startup script now generates ED25519 sshd(8) host
   keys if keys do not already exist when ``ssh_keygen_alg()`` is
   invoked.
...
FreeBSD?10.1-RELEASE is now available for the amd64, i386, ia64,
powerpc, powerpc64, sparc64, and armv6 architectures.
...
Additional UEFI-capable images are available for the amd64 (x86\_64)
architecture.
...
(x86\_64) and i386 (x86\_32) architectures in ``QCOW2``, ``VHD``, and
``VMDK`` disk image formats, as well as raw (unformatted) images.

FreeBSD?10.1-RELEASE is also available on these cloud hosting platforms:

-  Amazon??EC2™
   `FreeBSD/amd64 <https://aws.amazon.com/marketplace/pp/B00KSS55FY/>`__

-  Microsoft??Azure™
   `FreeBSD/amd64 <https://vmdepot.msopentech.com/Vhd/Show?vhdId=49971&version=51112>`__,?
   `FreeBSD/i386 <https://vmdepot.msopentech.com/Vhd/Show?vhdId=49972&version=51113>`__
...
support the release engineering activities for FreeBSD?10.1 including
