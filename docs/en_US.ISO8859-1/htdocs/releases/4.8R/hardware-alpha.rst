========================================
FreeBSD/alpha 4.8-RELEASE Hardware Notes
========================================
...
**Table of Contents**
1 `Introduction <#AEN11>`__
2 `Supported processors and motherboards <#AEN16>`__
2.1 `Overview <#AEN25>`__
2.2 `In general, what do you need to run FreeBSD on an
Alpha? <#AEN44>`__
2.3 `System-specific information <#AEN105>`__
2.3.1 `AXPpci33 (\`\`NoName'') <#AEN110>`__
2.3.2 `Universal Desktop Box (UDB or \`\`Multia'') <#AEN184>`__
2.3.3 `Personal Workstation (\`\`Miata'') <#AEN270>`__
2.3.4 `DEC3000 family (the \`\`Bird'' machines) <#AEN380>`__
2.3.5 `Evaluation Board 64 family <#AEN551>`__
...
(\`\`DaVinci'') <#AEN1272>`__
2.3.16 `AlphaServer 8200 and 8400 (\`\`TurboLaser'') <#AEN1302>`__
2.3.17 `Alpha Processor Inc. UP1000 <#AEN1345>`__
2.3.18 `Alpha Processor Inc. UP1100 <#AEN1394>`__
2.3.19 `Alpha Processor Inc. CS20, Compaq DS20L <#AEN1442>`__
2.3.20 `Compaq AlphaServer ES40 (\`\`Clipper'') <#AEN1489>`__
2.4 `Supported Hardware Overview <#AEN1526>`__
2.5 `Acknowledgments <#AEN1558>`__
3 `Supported Devices <#AEN1599>`__
3.1 `Disk Controllers <#AEN1605>`__
3.2 `Ethernet Interfaces <#ETHERNET>`__
3.3 `FDDI Interfaces <#AEN2903>`__
3.4 `ATM Interfaces <#AEN2913>`__
3.5 `Wireless Network Interfaces <#AEN2955>`__
3.6 `Miscellaneous Networks <#AEN3083>`__
3.7 `ISDN Interfaces <#AEN3093>`__
3.8 `Multi-port Serial Interfaces <#AEN3157>`__
3.9 `Audio Devices <#AEN3257>`__
3.10 `Camera and Video Capture Devices <#AEN3360>`__
3.11 `USB Devices <#USB>`__
3.12 `IEEE 1394 (Firewire) Devices <#FIREWIRE>`__
3.13 `Cryptographic Accelerators <#AEN3619>`__
3.14 `Miscellaneous <#AEN3644>`__
...
4.8-RELEASE on the Alpha/AXP hardware platform (also referred to as
FreeBSD/alpha 4.8-RELEASE). It lists devices known to work on this
...
.. raw:: html
    ...
    **Note:** You will see references to DEC, Digital Equipment
    ...
    refer to Compaq only. Given the fact that you will see the mix of
...
.. raw:: html
    ...
    **Note:** SRM commands will be in ``UPPER CASE``. Lower case input
    is also acceptable to SRM. Upper case is used for clarity.
...
.. raw:: html
    ...
    **Note:** Compaq has put information on the Web for Linux developers
    that is also very useful for FreeBSD users. Please check at `Linux
    Alpha Power tools <http://www.support.compaq.com/alpha-tools/>`__.
...
`pcm(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pcm&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
...
On 164SX you can have a maximum of 1 Gbyte of RAM. 4 regular 256MB PC133
ECC DIMMs are reported to work just fine. Whether 512MB DIMMs will also
work is currently unknown.
...
.. raw:: html
    ...
    **Note:** Contrary to expectation there is no ``cpu EV6`` defined
    ...
    mandatory to keep
    `config(8) <http://www.FreeBSD.org/cgi/man.cgi?query=config&sektion=8&manpath=FreeBSD+4.8-RELEASE>`__
    happy.
...
The on-board Adaptec is not bootable but works with FreeBSD 4.0 and
later as a datadisk-only SCSI bus.
...
in a 19" rack. This is medium iron, not a typical hobbyist system.
Rawhides are multi-CPU machines, up to 4 CPUs can be in a single
machine. Basic disk storage is housed in one or two StorageWorks shelves
at the bottom of the pedestal. The Rawhides intended for the NT market
are designated DIGITAL Server 7300 (5/400 CPU), DIGITAL Server 7305
(5/533 CPU). A trailing R on the part-number means a rackmount variant.
...
EDO RAM or synchronous DRAM. A fully populated Rawhide has 4 pairs of
memory modules. Given the choice use SDRAM for best performance. The
highest capacity memory board must be in memory slot 0. A mix of memory
board sizes is allowed. A mix of EDO and SDRAM is also reported as
working (assuming you don't try to mix EDO and SDRAM in one module
pair). A mix of EDO and SDRAM results in the *entire* memory subsystem
running at the slower EDO timing
...
sure all power is removed from the system remove the mains cables from
the system.
...
`de(4) <http://www.FreeBSD.org/cgi/man.cgi?query=de&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
(older driver) or
`dc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=dc&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
(newer driver). Some new SRM versions are known to recognize the Intel
8255x Ethernet chips as driven by the FreeBSD
`fxp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fxp&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
...
-  Adaptec
   ...
   variants
   (`ahc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ahc&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
   driver)
...
(`amr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=amr&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
firmware
(`mlx(4) <http://www.FreeBSD.org/cgi/man.cgi?query=mlx&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
.. raw:: html
   ...
   <div class="NOTE">
    ...
    Booting from these controllers is not supported due to SRM
    limitations. This list includes controllers sold by Digital/Compaq
    in Alpha systems in the StorageWorks family, e.g. KZPSC or KZPAC.
...
(`ncr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ncr&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
and
`sym(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sym&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
drivers)
...
(`isp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=isp&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
-  ATAPI IDE interface
   (`acd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=acd&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__)
...
(`sf(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sf&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
AMD PCnet NICs
(`lnc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=lnc&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
and
`pcn(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pcn&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
drivers)
...
NE2000 and clones
(`ed(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ed&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(`rl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=rl&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(`wb(4) <http://www.FreeBSD.org/cgi/man.cgi?query=wb&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)

-  Trendware TE100-PCIE

VIA Technologies VT3043 \`\`Rhine I'', VT86C100A \`\`Rhine II'', and
VT86C105/VT86C105M \`\`Rhine III'' Fast Ethernet NICs
(`vr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=vr&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(`sis(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sis&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(`ste(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ste&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)

-  D-Link DFE-550TX

SysKonnect SK-984x PCI Gigabit Ethernet cards
(`sk(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sk&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
drivers)
...
(`tl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=tl&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(`xl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=xl&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(3CR990) chipset
(`txp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=txp&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
chips
(`wx(4) <http://www.FreeBSD.org/cgi/man.cgi?query=wx&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__,
`gx(4) <http://www.FreeBSD.org/cgi/man.cgi?query=gx&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
and
`em(4) <http://www.FreeBSD.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
...
DEC DEFPA PCI
(`fpa(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fpa&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(`rp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=rp&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
-  ES1868, ES1869, ES1879 and ES1888
   (`sbc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sbc&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
   driver)
...
Host Controllers
(`ohci(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ohci&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
and
`uhci(4) <http://www.FreeBSD.org/cgi/man.cgi?query=uhci&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
drivers)
...
Keyboards
(`ukbd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ukbd&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
Mice
(`ums(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ums&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
(`ulpt(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ulpt&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
Storage
(`umass(4) <http://www.FreeBSD.org/cgi/man.cgi?query=umass&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
Floppy drives
(`fd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fd&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
...
-  PS/2 mice
   (`psm(4) <http://www.FreeBSD.org/cgi/man.cgi?query=psm&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
   driver)
...
(`ppc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ppc&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)

\`\`PC standard'' 8250, 16450, and 16550-based serial ports
(`sio(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sio&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__
driver)
