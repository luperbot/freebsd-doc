...
range. This change only applies to devices which do not have their
memory assigned by the BIOS. This change fixes the “bad Vcc” error of
CardBus bridges
(`pccbb(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pccbb&sektion=4&manpath=FreeBSD+6.0-RELEASE>`__).
[MERGED]
...
`boot0cfg(8) <http://www.FreeBSD.org/cgi/man.cgi?query=boot0cfg&sektion=8&manpath=FreeBSD+6.0-RELEASE>`__
...
Support for the OLDCARD subsystem has been removed. The NEWCARD system
is now used for all PCCARD device support.
...
The
`pbio(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pbio&sektion=4&manpath=FreeBSD+6.0-RELEASE>`__
...
driver now supports VESA (15, 16, 24, and 32 bit) modes. To enable this
feature, two kernel options ``SC_PIXEL_MODE`` and ``VESA`` (or
corresponding kernel module) are needed.
...
ARC-12\ ``xx`` series of SATA RAID controllers. [MERGED]
...
Read-only support for ReiserFS version 3 has been added. See
`mount\_reiserfs(8) <http://www.FreeBSD.org/cgi/man.cgi?query=mount_reiserfs&sektion=8&manpath=FreeBSD+6.0-RELEASE>`__
for details.
...
The
`mkuzip(8) <http://www.FreeBSD.org/cgi/man.cgi?query=mkuzip&sektion=8&manpath=FreeBSD+6.0-RELEASE>`__
utility, which compresses file system images for use with ``GEOM_UZIP``
`geom(4) <http://www.FreeBSD.org/cgi/man.cgi?query=geom&sektion=4&manpath=FreeBSD+6.0-RELEASE>`__
