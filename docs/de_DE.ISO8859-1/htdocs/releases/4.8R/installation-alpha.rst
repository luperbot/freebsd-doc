...
.. raw:: html
    ...
    FreeBSD/alpha 4.8-RELEASE, dabei wird besonderen Wert auf den Bezug
...
Befehl ``show           device`` k?nnen Sie herausfinden, welche Namen
der SRM f?r die Festplatten in Ihrem System benutzt:
...
Mit den Schaltern ``-file`` und ``-flags`` k?nnen Sie angeben, welcher
Kernel gebootet und welche Optionen gesetzt werden sollen:
