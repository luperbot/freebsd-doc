...
(\`\`DaVinci'') <#AEN1279>`__
2.3.16. `AlphaServer 8200 und 8400 (\`\`TurboLaser'') <#AEN1309>`__
2.3.17. `Alpha Processor Inc. UP1000 <#AEN1353>`__
2.3.18. `Alpha Processor Inc. UP1100 <#AEN1403>`__
2.3.19. `Alpha Processor Inc. CS20, Compaq DS20L <#AEN1451>`__
2.3.20. `Compaq AlphaServer ES40 (\`\`Clipper'') <#AEN1498>`__
2.4. `?bersicht ?ber die unterst?tzte Hardware <#AEN1535>`__
2.5. `Danksagung <#AEN1567>`__
3. `Unterst?tzte Ger?te <#AEN1608>`__
3.1. `Festplatten-Controller <#AEN1614>`__
3.2. `Netzwerkkarten <#ETHERNET>`__
3.3. `FDDI Karten <#AEN2909>`__
3.4. `ATM Karten <#AEN2919>`__
3.5. `Karten f?r drahtlose Netzwerke <#AEN2962>`__
3.6. `Diverse Netzwerkkarten <#AEN3090>`__
3.7. `ISDN Karten <#AEN3100>`__
3.8. `Serielle Multi-Port Karten <#AEN3164>`__
3.9. `Audio Hardware <#AEN3264>`__
3.10. `Kameras, TV-Karten, etc <#AEN3367>`__
3.11. `USB Ger?te <#USB>`__
3.12. `IEEE 1394 (Firewire) Ger?te <#FIREWIRE>`__
3.13. `Verschl?sselungs-Hardware <#AEN3626>`__
3.14. `Diverses <#AEN3651>`__
...
4.8-RELEASE auf Alpha/AXP Systemen (meistens einfach FreeBSD/alpha
4.8-RELEASE genannt). Hier finden Sie zum einen alle die Ger?te, von
...
.. raw:: html
    ...
    **Anmerkung:** SRM Befehle werden in ``GROSSBUCHSTABEN``
    dargestellt. SRM akzeptiert auch Kleinbuchstaben, die Verwendung von
...
`pcm(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pcm&sektion=4&manpath=FreeBSD+4.8-RELEASE>`__-Treiber
...
Eingabeaufforderung ``set pci_device_override`` eingeben. Wenn Ihre
Daten danach mysteri?se Fehler aufweisen, d?rfen Sie sich allerdings
nicht beschweren.
...
f?r den angep?aten Kernel die folgende Zeile stehen:
...
.. raw:: html
    ...
    **Anmerkung:** Obwohl Sie es vielleicht erwartet haben, ist es nicht
    notwendig, ``cpu EV6`` anzugeben. Das ``cpu EV5`` ist nur notwendig,
    damit
...
SRM Variable ``usb_enable`` auf ``on`` steht. Sie k?nnen die Einstellung
mit dem folgenden Befehl ?ndern:
...
Sobald Sie FreeBSD starten, beachtet es die Einstellung f?r ``CONSOLE``
und alle Meldungen beim Systemstart und das Login erscheinen auf der
seriellen Konsole.
...
in die mit ``0`` gekennzeichneten Steckpl?tze einsetzen. Au?erdem m?ssen
Sie die Steckpl?tze \`\`der Reihe nach'' benutzen, also erst Bank 0,
dann Bank 1, und so weiter.
...
Obwohl Sie es vielleicht erwartet haben, ist es nicht notwendig,
``cpu EV6`` anzugeben. Das ``cpu EV5`` ist nur notwendig, damit
`config(8) <http://www.FreeBSD.org/cgi/man.cgi?query=config&sektion=8&manpath=FreeBSD+4.8-RELEASE>`__
nicht meckert.
