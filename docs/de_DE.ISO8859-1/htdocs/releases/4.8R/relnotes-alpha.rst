...
Wenn Sie FreeBSD auf einer AlphaServer 8200 oder 8400 \`\`TurboLaser''
benutzen, m?ssen Sie nach der Installation den getty f?r die serielle
Konsole manuell aktiviert. Wenn Sie das nicht tun, sieht es so aus, als
h?tte sich Ihre Maschine aufgeh?ngt, weil Sie keine Login-Aufforderung
erhalten. Den notwendigen Eintrag finden Sie in ``/etc/ttys`` bei
``zs0``.
