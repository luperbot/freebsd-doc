...
.. raw:: html
    ...
    FreeBSD/i386 4.8-RELEASE, dabei wird besonderen Wert auf den Bezug
...
Schwerpunkt liegt dabei auf dem Bezug der FreeBSD 4.8-RELEASE
...
Wenn Sie FreeBSD auf einem i386 System benutzen wollen, mu? das System
mindestens ?ber einen 80386 Prozessor verf?gen. Das
Installationsprogramm
`sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+4.8-RELEASE>`__
ben?tigt 16 MByte; nach der Installation l?uft FreeBSD mit einem
entsprechenden Kernel mit 4-8 MByte RAM. Sie brauchen mindestens 100
MByte freien Platz auf der Festplatte f?r die absolute
Minimalinstallation; ein realistischerer Wert liegt bei 250-350 MByte.
Weiter unten finden Sie Hinweise, wie Sie existierende Partitionen
verkleinern k?nnen, um Platz f?r FreeBSD zu schaffen.
...
``release``\ ``/floppies/mfsroot.flp`` von ftp://ftp.FreeBSD.org/ oder
einem der vielen Mirrors beziehen. Eine Liste aller Mirrors finden Sie
im Kapitel `FTP
...
-  Wenn Ihr System direkt von der CD booten kann (eine Option, die Sie
   normalerweise explizit auf Ihrem SCSI-Controller oder im PC-BIOS
   aktivieren m?ssen) und Sie diese Option aktiviert haben, dann reicht
   es aus, die CD in das CDROM Laufwerk einzulegen und das System neu zu
   starten.

-  Alternativ k?nnen Sie einen Satz FreeBSD Bootdisketten verwenden, die
   notwendigen Zutaten finden Sie im bei jeder FreeBSD Distribution im
   Verzeichnis ``floppies/``. Wenn Sie eine DOS-Shell benutzen, k?nnen
   Sie das Script ``makeflp.bat`` verwenden. Weitere Informationen zu
   diesem Thema finden Sie im Kapitel `Abschnitt 1.3 <#FLOPPIES>`__.
   Danach m?ssen Sie nur noch von der ersten Diskette booten, um die
   FreeBSD-Installation zu starten.

Wenn Ihr Rechner kein CDROM Laufwerk hat (oder nicht von nicht von CD
booten kann), k?nnen Sie FreeBSD auch direkt ?ber das Internet
installieren, wenn Sie PPP, SLIP oder eine feste Verbindung haben. In
diesem Fall m?ssen Sie ebenfalls die beiden FreeBSD Bootdisketten aus
den Dateien ``floppies/kern.flp`` und ``floppies/mfsroot.flp`` erzeugen,
wie in `Abschnitt 1.3 <#FLOPPIES>`__ beschrieben. Booten Sie Ihren
Rechner von der Diskette mit ``kern.flp``; und legen Sie die Diskette
mit ``mfsroot.flp`` ein, sobald Sie dazu aufgefordert werden. Weitere
Informationen zu dieser Art der Installation finden Sie in `Abschnitt
1.5.5 <#FTPNFS>`__.
...
1.6.1. `Hilfe! Ich habe keinen Platz frei! Mu? ich alles anderen
l?schen? <#Q1.6.1.>`__
...
Beachten Sie bitte, da? die zweite Partition aus der ersten \`\`clont'',
Sie werden also zwei prim?re DOS Partitionen vorfinden, obwohl Sie
vorher nur eine hatten. Keine Panik, Sie k?nnen die zus?tzliche prim?re
DOS Partition einfach l?schen (aber achten Sie darauf, da? Sie die
richtige Partition l?schen, indem Sie auf die Gr??e der Partition
achten).

**FIPS** kann nicht f?r NTFS Partitionen benutzt werden. F?r diese
Partitionen brauchen Sie ein kommerzielles Programm wie **Partition
Magic**. Es mag zwar hart klingen, aber das ist halt der Preis, den Sie
zahlen n?ssen, wenn eine Windows Partition Ihre gesamte Festplatte
belegt und Sie nicht alles neu installieren wollen.
...
ersetzen Sie ``da`` durch ``ad``. Abgesehen vom Namen k?nnen Sie auf
logische Laufwerke genauso wie auf jedes andere DOS-Laufwerk zugreifen:
...
benutzen. Allerdings ist dieses Programm noch nicht vollst?ndig und es
gibt noch einige Probleme damit. Wenn Sie interessiert sind, an diesem
Programm mitzuarbeiten, schreiben Sie eine Mail an die Mailingliste
...
.. raw:: html
    ...
    **Wichtig:** Diese Anleitung geht davon aus, da? Sie das
    `sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+4.8-RELEASE>`__-Programm
    der *NEUEN* FreeBSD-Version zur Aktualisierung verwenden. Die
    Verwendung der falschen
    `sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+4.8-RELEASE>`__-Version
    ...
    `sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+4.8-RELEASE>`__-Programms
...
Distributionen. Allerdings sollten Sie die Distribution ``bin`` auf
...
Was bedeutet diese ``bios_drive:interface(unit,partition)kernel_name``
...
FreeBSD liegt auf BIOS Platte 1, der Typ der Platte ist ``wd``, und
FreeBSD vergibt die Nummer 2. In diesem Fall m?ssen Sie folgendes
eingeben:
...
.. raw:: html
    ...
    **Anmerkung:** Erg?nzungen zu diesem Abschnitt schicken Sie bitte an
...
4.3.5. `Ich habe meinen IBM ThinkPad (tm) Laptop von der
Installationsdiskette gebootet, und die Tastaturbelegung ist v?llig
konfus. <#Q4.3.5.>`__
4.3.6. `Wenn ich von der Installationsdiskette boote, erhalte ich nur
die folgende Meldung, danach passiert nichts mehr. Ich kann auch keine
Tastatureingaben machen. <#Q4.3.6.>`__
4.3.7. `Ich habe ein Matsushita/Panasonic CR-522, ein
...
angebe. <#Q4.3.7.>`__
4.3.8. `Ich versuche, von Band zu installieren, aber ich erhalte nur die
folgende Meldung: <#Q4.3.8.>`__
4.3.9. `Ich habe FreeBSD auf meinem Rechner installiert, aber beim
Neustart bleibt das System stehen. Die letzte Meldung ist: <#Q4.3.9.>`__
4.3.10. `Meine Intel EtherExpress 16 wird nicht erkannt. <#Q4.3.10.>`__
4.3.11. `Wenn ich FreeBSD auf meinem EISA HP Netserver installiere, wird
der eingebaute AIC-7xxx SCSI Controller nicht erkannt. <#Q4.3.11.>`__
4.3.12. `Mein Panasonic AL-N1 oder Rios Chandler Pentium Rechner h?ngt
sich auf, obwohl das Installationsprogramm noch gar nicht gestartet
ist. <#Q4.3.12.>`__
4.3.13. `Ich habe diesen CMD640 IDE Controller, der angeblich
\`\`broken'' ist. <#Q4.3.13.>`__
4.3.14. `Auf meinem Compaq Aero Notebook erhalte ich die Meldung \`\`No
floppy devices found! Please check ...'', wenn ich von Diskette
installieren will. <#Q4.3.14.>`__
4.3.15. `Wenn ich das erste Mal von meinem neuen Board mit Intel AL440LX
Chipsatz (\`\`Atlanta'') boote, bricht der Startvorgang mit der Meldung
``Read Error`` ab. <#Q4.3.15.>`__
4.3.16. `Wenn ich FreeBSD auf einem Dell Poweredge XE installiere, wird
der Dell-eigene RAID Controller DSA (Dell SCSI Array) nicht
erkannt. <#Q4.3.16.>`__
4.3.17. `Meine Netzwerkkarte wird als AMD PCnet-FAST (oder ?hnliche)
erkannt, funktioniert aber nicht (betrifft u.a. die eingebauten
Netzwerkkarten in den IBM Netfinity 5xxx oder 7xxx
Systemen) <#Q4.3.17.>`__
4.3.18. `Ich verwende eine Netzwerkkarte vom Typ IBM EtherJet PCI, die
...
Netzwerk. <#Q4.3.18.>`__
4.3.19. `Wenn ich w?hrend einer Installation auf einer IBM Netfinity
3500 das Netzwerk konfiguriere, h?ngt sich das System
auf. <#Q4.3.19.>`__
4.3.20. `Wenn ich FreeBSD auf einer Festplatte konfiguriere, die an
einem Mylex PCI RAID Controller abgeschlossen ist, kann ich das System
nicht neu starten (Abbruch mit der Meldung
``read error``). <#Q4.3.20.>`__
...
f?r den IRQ den Wert ``-1`` eingeben. Damit teilen Sie dem Kernel mit,
da? er die \`soft' Konfiguration benutzen soll.
...
\ **4.3.6.** Wenn ich von der Installationsdiskette boote, erhalte ich
...
Double-Speed Laufwerke CR-562 und CR-563 identisch sind.
...
\ **4.3.8.** Ich versuche, von Band zu installieren, aber ich erhalte
...
erhalten, m?ssen Sie ``-c`` eingeben. Damit starten Sie UserConfig
(siehe `Abschnitt 4.1 <#REPAIRING>`__ weiter oben). Benutzen Sie den
Befehl ``disable``, um alle anderen Treiber abzuschalten, die auch auf
...
\ **4.3.11.** Wenn ich FreeBSD auf meinem EISA HP Netserver installiere,
...
``-c`` booten, um UserConfig zu starten. Sie brauchen den CLI-Modus,
*nicht* den Visual Modus (auch wenn er sch?ner aussieht). Geben Sie den
...
\ **4.3.12.** Mein Panasonic AL-N1 oder Rios Chandler Pentium Rechner
...
Anweisungen ``i586_copyout`` und ``i586_copyin`` nicht. Damit diese
...
\ **4.3.14.** Auf meinem Compaq Aero Notebook erhalte ich die Meldung
...
\ **4.3.15.** Wenn ich das erste Mal von meinem neuen Board mit Intel
...
\ **4.3.16.** Wenn ich FreeBSD auf einem Dell Poweredge XE installiere,
...
\ **4.3.17.** Meine Netzwerkkarte wird als AMD PCnet-FAST (oder
...
\ **4.3.18.** Ich verwende eine Netzwerkkarte vom Typ IBM EtherJet PCI,
...
\ **4.3.19.** Wenn ich w?hrend einer Installation auf einer IBM
