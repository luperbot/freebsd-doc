...
.. raw:: html
    ...
    FreeBSD/pc98 5.3-RELEASE, dabei wird besonderen Wert auf den Bezug
...
Wenn Sie FreeBSD auf einem NEC PC-98x1 System installieren wollen, mu?
das System mindestens ?ber einen 486 Prozessor und 8 MByte RAM verf?gen.
F?r den Betrieb mit einem angepa?ten Kernel reicht auch ein 386
Prozessor und 7 MByte Speicher. Sie brauchen mindestens 150 MByte freien
Platz auf der Festplatte f?r die Minimalinstallation. Weiter unten
finden Sie Hinweise, wie Sie existierende Partitionen verkleinern
k?nnen, um Platz f?r FreeBSD zu schaffen.
...
``boot.flp`` und ``kernX.flp``, die f?r 1.44 MByte Diskettenlaufwerke
gedacht sind; oder um die Disketten-Images ``boot-small.flp`` und
``kern-small.flp``, die f?r 1.2 MByte Diskettenlaufwerke gedacht sind.
...
Dazu m?ssen Sie ``rawrite.exe`` und ``boot.flp`` in ein Verzeichnis
...
-  Verwenden Sie einen Satz FreeBSD Bootdisketten, die notwendigen
   Zutaten finden Sie bei jeder FreeBSD Distribution im Verzeichnis
...
Verbindung haben. In diesem Fall m?ssen Sie ebenfalls die FreeBSD
Bootdisketten aus den Dateien ``floppies/boot.flp`` und
``floppies/kernX.flp`` erzeugen, wie in `Abschnitt 1.3 <#FLOPPIES>`__
beschrieben. Booten Sie Ihren Rechner von der Diskette mit ``boot.flp``;
und legen Sie die Diskette mit ``kernX.flp`` ein, sobald Sie dazu
