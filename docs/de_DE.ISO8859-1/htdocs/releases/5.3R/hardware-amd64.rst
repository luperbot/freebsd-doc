...
5.3-RELEASE auf AMD64 Systemen (meistens einfach FreeBSD/amd64
...
Seit Mitte 2003 unterst?tzt FreeBSD/amd64 die AMD64 (\`\`Hammer'') und
Intel EM64T Architektur. FreeBSD/amd64 ist nun eine der Tier-1
Plattformen (das hei?t voll unterst?tzt), die unter allen
Gesichtspunkten des FreeBSD-Betriebssystems (wie Installation und
Entwicklungsumgebungen) als produktionsreif angesehen werden.

F?r diese Architektur gibt es zwei Namen: AMD64 (AMD) und Intel EM64T
(Extended Memory 64-bit Technology). Der 64-Bit-Modus beider
Architekturen ist weitgehend gleich, so da? beide Architekturen unter
FreeBSD/amd64 laufen.

Zur Zeit werden folgende Prozessoren unterst?tzt:

-  AMD Athlon64 (\`\`Clawhammer''),

-  AMD Opteron (\`\`Sledgehammer''),

-  Intel 64-bit Xeon (\`\`Nacona''). Dieser Prozessor wird mit 90 nm
   Fertigungstechnik hergestellt und mit 2,80 GHz bis 3,60 GHz (FSB
   800 MHz) betrieben. Es werden Intel E7520/E7525/E7320 Chips?tze
   verwendet.

-  Intel Pentium 4 Prozessor mit Intel EM64T (\`\`Prescott''). Dieser
   Prozessor wird mit 90 nm Fertigungstechnik hergestellt und mit
   3,20F/3,40F/3,60F GHz und Intel 925X Express Chips?tzen betrieben.
   Die zugeh?rigen S-Spec Zahlen sind SL7L9, SL7L8, SL7LA, SL7NZ, SL7PZ
   und SL7PX. Beachten sie, da? Prozessoren aus der 5xx-Serie EM64T
   nicht unterst?tzen.

Intel EMT64 ist eine erweiterte Version der IA-32 (x86) Architektur. Sie
unterscheidet sich von der IA-64 (Itanium) Architektur, die von
FreeBSD/ia64 unterst?tzt wird. In alter Dokumentation von Intel wird
EM64T auch als \`\`64-bit extension technology'' oder \`\`IA-32e''
bezeichnet.

Der gr??te bisher getestete Speicherausbau war 8 GByte. SMP wird jetzt
unterst?tzt und ist schon ausreichend stabil.
...
FreeBSD/amd64 ist immer noch eine recht junge Plattform f?r FreeBSD. Der
