...
2. `Unterst?tzte Prozessoren und Mainboards <#SUPPORT-PROC>`__
2.1. `?bersicht <#AEN33>`__
2.2. `Was braucht man im Allgemeinen um FreeBSD auf einer Alpha zu
nutzen? <#AEN48>`__
2.3. `Spezifische Informationen f?r einzelne Systeme <#AEN110>`__
2.3.1. `AXPpci33 (\`\`NoName'') <#AEN115>`__
2.3.2. `Universal Desktop Box (UDB oder \`\`Multia'') <#AEN190>`__
2.3.3. `Personal Workstation (\`\`Miata'') <#AEN280>`__
2.3.4. `Evaluation Board 64 Systeme <#AEN389>`__
2.3.5. `Evaluation Board 164 (\`\`EB164, PC164, PC164LX, PC164SX'')
Familien <#AEN444>`__
2.3.6. `AlphaStation 200 (\`\`Mustang'') und 400 (\`\`Avanti'')
Familien <#AEN510>`__
2.3.7. `AlphaStation 500 und 600 (\`\`Alcor'' und \`\`Maverick'' f?r
EV5, \`\`Bret'' f?r EV56) <#AEN578>`__
2.3.8. `AlphaServer 1000 (\`\`Mikasa''), 1000A (\`\`Noritake'') und 800
(\`\`Corelle'') <#AEN688>`__
2.3.9. `DS10/VS10/XP900 (\`\`Webbrick'') / XP1000 (\`\`Monet'') / DS10L
(\`\`Slate'') <#AEN747>`__
2.3.10. `DS20/DS20E (\`\`Goldrush'') <#AEN900>`__
2.3.11. `AlphaPC 264DP / UP2000 <#AEN970>`__
2.3.12. `AlphaServer 2000 (\`\`DemiSable''), 2100 (\`\`Sable''), 2100A
(\`\`Lynx'') <#AEN1016>`__
2.3.13. `AlphaServer 4x00 (\`\`Rawhide'') <#AEN1089>`__
2.3.14. `AlphaServer 1200 (\`\`Tincup'') und AlphaStation 1200
(\`\`DaVinci'') <#AEN1122>`__
2.3.15. `AlphaServer 8200 und 8400 (\`\`TurboLaser'') <#AEN1152>`__
2.3.16. `Alpha Processor Inc. UP1000 <#AEN1196>`__
2.3.17. `Alpha Processor Inc. UP1100 <#AEN1246>`__
2.3.18. `Alpha Processor Inc. CS20, Compaq DS20L <#AEN1294>`__
2.3.19. `Compaq AlphaServer ES40 (\`\`Clipper'') <#AEN1341>`__
2.4. `?bersicht ?ber die unterst?tzte Hardware <#AEN1378>`__
2.5. `Danksagung <#AEN1410>`__
...
5.3-RELEASE auf Alpha/AXP Systemen (meistens einfach FreeBSD/alpha
...
`pcm(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pcm&sektion=4&manpath=FreeBSD+5.3-RELEASE>`__-Treiber
...
Die Audio-Hardware nutzt Port 0x530, IRQ 9 und DRQ 3. Sie m?ssen in
``device.hints`` zus?tzlich noch flags 0x15 angeben.
...
Hostadaptern der Adaptec 2940-Familie booten. Ein Test mit einem
Adaptec 3940UW ergab, da? dieser Adapter allerdings nicht unterst?tzt
wird. Der SRM der 164SX erkennt Intel 8255x Ethernetkarten als eia. Mit
dieser Netzwerkkarte kann ?ber das Netzwerk gebootet werden.
...
eingesetzt wird. Leider ist die 164SX sehr w?hlerisch, was DIMMs
anbelangt. Stellen Sie sich darauf ein, zu experimentieren.
...
Die Audio-Hardware nutzt Port 0x530, IRQ 10 und DRQ 3. Sie m?ssen in
``device.hints`` zus?tzlich noch flags 0x10011 angeben.
...
2.3.7. AlphaStation 500 und 600 (\`\`Alcor'' und \`\`Maverick'' f?r EV5, \`\`Bret'' f?r EV56)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
0x530 und DRQ 0 einzustellen. Sie m?ssen diese Werte ebenfalls in
``device.hints`` angeben, dazu kommt noch die Angabe flags 0x10011.
...
Busmaster DMA wird nur auf dem ersten IDE-Anschlu? unterst?tzt. Das
...
\`\`PC Standard'', werden unterst?tzt.
...
.. raw:: html
   ...
   <div class="NOTE">
    ...
    Aufgrund von Einschr?nkungen im SRM k?nnen Sie von diesen
    Hostadaptern nicht booten. Von DAC960 Controller f?r Alpha-Systeme,
    die von Digital/Compaq als Teil der StorageWorks Familie verkauft
    wurden (wie z.B. der KZPSC oder KZPAC), k?nnen sie via SRM booten.
    Allerdings benutzten diese Karten die Firmware-Version 2.x. Es ist
    nicht bekannt, ob es auch mit einer neueren Firmware, via SRM von
    diesen Karten zu booten.
