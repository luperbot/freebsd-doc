...
.. raw:: html
    ...
    5.1-RELEASE. Es enth?lt einige Hinweise, wie Sie FreeBSD beziehen
...
5.1-RELEASE und repr?sentiert den aktuellen Zustand im Entwicklungszweig
5-CURRENT.
...
UltraSPARC Systeme (sparc64). Versionen f?r die Plattformen IA64 (ia64),
PowerPC (powerpc) und AMD \`\`Hammer'' (amd64) sind ebenfalls in der
Entwicklung. FreeBSD unterst?tzt viele verschiedene Ger?te und
...
installieren zu k?nnen. Die ?ber 9,000 Ports (vom Editor ?ber
...
`send-pr(1) <http://www.FreeBSD.org/cgi/man.cgi?query=send-pr&sektion=1&manpath=FreeBSD+5.1-release>`__
...
-  ``EARLY.TXT``: Spezielle Hinweise f?r die ersten Anwender von FreeBSD
   5.1-RELEASE. Wenn dies Ihre erste Begegnung mit FreeBSD 5-CURRENT ist
...
-  ``RELNOTES.TXT``: Die begleitenden Informationen zu dieser Release
   ...
   und die ?nderungen seit der letzten Version (FreeBSD 5.0-RELEASE).
...
`man(1) <http://www.FreeBSD.org/cgi/man.cgi?query=man&sektion=1&manpath=FreeBSD+5.1-release>`__
...
`tuning(7) <http://www.FreeBSD.org/cgi/man.cgi?query=tuning&sektion=7&manpath=FreeBSD+5.1-release>`__
(Hinweise, wie sie die Performance Ihres Systems verbessern k?nnen),
`security(7) <http://www.FreeBSD.org/cgi/man.cgi?query=security&sektion=7&manpath=FreeBSD+5.1-release>`__
(eine Einf?hrung in das Thema Sicherheit unter FreeBSD), und
`style(9) <http://www.FreeBSD.org/cgi/man.cgi?query=style&sektion=9&manpath=FreeBSD+5.1-release>`__
