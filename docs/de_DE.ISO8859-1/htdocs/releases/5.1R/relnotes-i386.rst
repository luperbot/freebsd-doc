...
Der Treiber tw f?r die TW-523 Systeme zur Steuerung ?ber das Stromnetz
wurde entfernt; er wurde von den X-10 Produkten genutzt. Der Treiber
funkioniert zur Zeit nicht und k?nnte nur mit hohem Aufwand an 5-CURRENT
angepa?t werden. Die zugeh?rigen Programme xten und xtend wurden
ebenfalls entfernt.
...
Kernel Threads k?nnen jetzt einen Lazy Switch Mechanismus nutzen. Damit
kann der Overhead von kurzen Kontextwechseln verringert werden, die
keinen anderen Prozess aufrufen; dazu geh?rt z.B. die Interrupt-Handler.
Um diesen Mechanismus zu nutzen, mu? in der Konfigurationsdatei f?r den
angepa?ten Kernel die Zeile ``options LAZY_SWITCH`` stehen.
...
``SMP`` Kernel bieten jetzt minimale Unterst?tzung f?r HyperThreading
(HTT). Die logischen Prozessoren werden vom Scheduler wie zus?tzliche
physikalische Prozessoren behandelt. Da dieses sehr einfache Vorgehen zu
schlechter Performance f?hren kann, werden die logischen CPUs beim
Systemstart standardm??ig deaktiviert. Sie k?nnen mit der
sysctl-Variable ``machdep.hlt_logical_cpus`` aktiviert werden. Es ist
au?erdem m?glich, jede CPU anzuhalten, die sich im Idle-Loop befindet,
dazu dient die sysctl-Variable ``machdep.hlt_cpus``. Weitere
Informationen finden Sie in
...
.. raw:: html
    ...
    **Anmerkung:** Einige andere FreeBSD Versionen wie 4.8-RELEASE und
    die ersten Versionen von 5.0-CURRENT unterst?tzten HTT nur, wenn in
    der Konfigurationsdatei f?r den angepa?ten Kernel die Zeile
    ``options HTT`` stand. Diese Zeile wird nicht mehr ben?tigt.

.. raw:: html

   </div>

Auf allen Intel-Prozessoren ab Pentium Pro wird jetzt die Physical
Address Extension (PAE) unterst?tzt. Dadurch k?nnen in einem System bis
zu 64 GByte Speicher genutzt werden, allerdings ?ndern sich dadurch die
Begrenzungen f?r die maximale Speichergr??e eines Prozesses (oder das
FreeBSD Kernels) nicht. Weitere Informationen finden Sie in
`pae(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pae&sektion=4&manpath=FreeBSD+5.1-release>`__.
Die Arbeiten wurden von der DARPA und den Network Associates
Laboratories finanziert.
...
Die beiden Teile des Bootloaders (``boot1`` und ``boot2``) wurden in
einer Datei ``boot`` zusammengefa?t, dadurch kommt es zu Vereinfachungen
bei Programmen, die den Bootloader schreiben oder bearbeiten m?ssen.
...
Auf Grund von Platzproblemen kann der i386 Bootloader den Kernel nur vom
root-Dateisystem laden, wenn dieses maximal 1.5 TByte gro? ist.
...
unterst?tzt Netzwerkkarten auf Basis des RealTek RTL8150 USB to Fast
Ethernet Chips.

Der neue Treiber
`sbsh(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sbsh&sektion=4&manpath=FreeBSD+5.1-release>`__
unterst?tzt das SBNI16 SHDSL Modem der Firma Grant. [MERGED]
...
Der neue Treiber ips unterst?tzt die IBM (beziehungsweise jetzt Adaptec)
ServeRAID Systeme.
...
genutzen Module ``atspeaker.ko`` und ``pcspeaker.ko`` wurden in
``speaker.ko`` zusammengefa?t.
...
.. raw:: html
    ...
    **Anmerkung:** Auf Grund von Einschr?nkungen im i386 Bootloader darf
    das root-Dateisystem nur maximal 1.5 TByte gro? sein.
...
Die Bibliothek ``libkse`` unterst?tzt die POSIX Threads auf Basis von
KSE und wird jetzt standardm??ig erzeugt und installiert. Diese
Bibliothek unterst?tzt zur Zeit M:N-Threading. Sowohl Threads auf
System-Ebene als auch auf Proze?-Ebene werden unterst?tzt; au?erdem ist
es m?glich, die Anzahl gleichzeitig laufender Threads abzufragen und zu
setzen. Diese Anzahl wird von der Bibliothek standardm??ig auf die
Anzahl der CPUs gesetzt. Jeder gleichzeitig laufende Thread entspricht
einer KSE, und alle Threads auf Proze?-Ebene laufen innerhalb dieser
KSE. Jeder Thread auf System-Ebene erh?lt zus?tzlich dazu eine eigene
KSE. ``libkse`` ist zwar immer noch in Arbeit, kann aber bereits als
Ersatz f?r die Thread-Bibliothek ``libc_r`` genutzt werden, dazu mu?
beim Binden des Programms lediglich ``-lkse`` statt ``-pthread``
verwendet werden.
...
Die Kernel-Module f?r DRM wurden aktualisiert. Der neue Stand entspricht
einer Zwischenversion aus dem DRI CVS Repository, der zwar ungef?hr
XFree86 4.3.0 entspricht, aber einige zus?tzliche Fehlerkorrekturen
bietet.
...
.. raw:: html
    ...
    **Anmerkung:** Es wurde bekannt, da? **GCC** falschen Code erzeugt,
    wenn die Option ``-march=pentium4`` gesetzt ist. Um dieses Problem
    zu umgehen, wird automatisch ``-march=pentium3`` genutzt, falls die
    Makefile-Variable ``CPUTYPE=p4`` aktiviert wird (zum Beispiel in
    `make.conf(5) <http://www.FreeBSD.org/cgi/man.cgi?query=make.conf&sektion=5&manpath=FreeBSD+5.1-release>`__).
    Wir gehen davon aus, da? das Problem behoben wird, sobald GCC 3.3 in
    das Basissystem aufgenommen wird.
