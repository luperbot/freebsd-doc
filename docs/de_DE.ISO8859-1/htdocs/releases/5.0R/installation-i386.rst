...
.. raw:: html
    ...
    FreeBSD/i386 5.0-CURRENT, dabei wird besonderen Wert auf den Bezug
...
Schwerpunkt liegt dabei auf dem Bezug der FreeBSD 5.0-CURRENT
...
mindestens ?ber einen 386 Prozessor (es tut uns ja leid, aber die 286
CPU wird nicht unterst?tzt) und mindestens 5 MByte RAM f?r die
Installation und 4 MByte RAM f?r den Betrieb verf?gen. Sie brauchen
mindestens 100 MByte freien Platz auf der Festplatte f?r die
Minimalinstallation. Weiter unten finden Sie Hinweise, wie Sie
existierende Partitionen verkleinern k?nnen, um Platz f?r FreeBSD zu
schaffen.
...
In einigen F?llen brauchen sie auch das dritte Image ``drivers.flp``,
dies h?ngt von Ihrer Hardware ab.

Wenn Sie diese Images ?ber das Internet beziehen wollen, brauchen Sie
nur Dateien ``release``\ ``/floppies/kern.flp``,
``release``\ ``/floppies/mfsroot.flp`` und
``release``\ ``/floppies/drivers.flp`` von
http://snapshots.jp.FreeBSD.org/ oder einem der vielen Mirrors beziehen.
Eine Liste aller Mirrors finden Sie im Kapitel `FTP
...
nicht ganz normal auf eine DOS- oder UFS-Diskette kopieren, sondern
m?ssen ein spezielles Programm verwenden. Wenn Sie DOS benutzen,
brauchen Sie das Programm ``rawwrite.exe``, das Sie im Verzeichnis
``tools`` der CDROM bzw. auf dem FreeBSD FTP Server finden. Wenn Sie
UNIX benutzen, brauchen Sie
`dd(1) <http://www.FreeBSD.org/cgi/man.cgi?query=dd&sektion=1&manpath=FreeBSD+5.0-current>`__.
...
brauchen Sie ein kommerzielles Programm wie **Partition Magic**.
...
'FreeBSD-emulation' ``<freebsd-emulation@FreeBSD.org>``.
...
.. raw:: html
    ...
    **Wichtig:** Diese Anleitung geht davon aus, da? Sie das
    `sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+5.0-current>`__-Programm
    der *NEUEN* FreeBSD-Version zur Aktualisierung verwenden. Die
    Verwendung der falschen
    `sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+5.0-current>`__-Version
    ...
    `sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+5.0-current>`__-Programms
...
.. raw:: html
    ...
    **Warnung:** Sie sollten nicht versuchen, Ihr System mit der
    ...
    von FreeBSD 4-STABLE auf FreeBSD 5.0-CURRENT zu aktualisieren.
...
4.2.2. `Meine alte ISA-Karte wurde nicht mehr erkannt, in ?lteren
FreeBSD-Versionen funktionierte sie. Woran liegt das? <#Q4.2.2.>`__
4.2.3. `Wenn ich das System nach der FreeBSD-Installation zum ersten von
der Festplatte starte, wird der Kernel geladen und sucht nach Hardware,
aber danach kommt eine Meldung wie: <#Q4.2.3.>`__
4.2.4. `Wenn ich nach der FreeBSD-Installation zum ersten Mal von der
Festplatte booten will, komme ich nur bis zum ``F?`` Prompt des
Bootmanagers. <#Q4.2.4.>`__
4.2.5. `Der mcd(4) Treiber glaubt, der habe ein Ger?t gefunden, dadurch
funktioniert meine Intel EtherExpress Netzwerkkarte nicht. <#Q4.2.5.>`__
4.2.6. `Das System erkennt meine ed(4) Netzwerkkarte, aber ich erhalte
st?ndig die Meldung device timeout. <#Q4.2.6.>`__
4.2.7. `Ich habe meinen IBM ThinkPad (tm) Laptop von der
Installationsdiskette gebootet, und die Tastaturbelegung ist v?llig
konfus. <#Q4.2.7.>`__
4.2.8. `Meine Intel EtherExpress 16 wird nicht erkannt. <#Q4.2.8.>`__
4.2.9. `Wenn ich FreeBSD auf meinem EISA HP Netserver installiere, wird
der eingebaute AIC-7xxx SCSI Controller nicht erkannt. <#Q4.2.9.>`__
4.2.10. `Mein Panasonic AL-N1 oder Rios Chandler Pentium Rechner h?ngt
sich auf, obwohl das Installationsprogramm noch gar nicht gestartet
ist. <#Q4.2.10.>`__
4.2.11. `Ich habe diesen CMD640 IDE Controller, der angeblich
\`\`broken'' ist. <#Q4.2.11.>`__
4.2.12. `Auf meinem Compaq Aero Notebook erhalte ich die Meldung \`\`No
floppy devices found! Please check ...'', wenn ich von Diskette
installieren will. <#Q4.2.12.>`__
4.2.13. `Wenn ich FreeBSD auf einem Dell Poweredge XE installiere, wird
der Dell-eigene RAID Controller DSA (Dell SCSI Array) nicht
erkannt. <#Q4.2.13.>`__
4.2.14. `Ich verwende eine Netzwerkkarte vom Typ IBM EtherJet PCI, die
...
Dabei steht ``foo`` f?r den Namen des Treibers, den Sie aktivieren
wollen. Um diese ?nderung permanent zu machen, m?ssen Sie die Datei
...
\ **4.2.3.** Wenn ich das System nach der FreeBSD-Installation zum
...
FreeBSD liegt auf BIOS Platte 1, der Typ der Platte ist ``ad``, und
...
\ **4.2.4.** Wenn ich nach der FreeBSD-Installation zum ersten Mal von
...
\ **4.2.5.** Der mcd(4) Treiber glaubt, der habe ein Ger?t gefunden,
...
\ **4.2.6.** Das System erkennt meine ed(4) Netzwerkkarte, aber ich
...
\ **4.2.9.** Wenn ich FreeBSD auf meinem EISA HP Netserver installiere,
...
\ **4.2.10.** Mein Panasonic AL-N1 oder Rios Chandler Pentium Rechner
...
\ **4.2.12.** Auf meinem Compaq Aero Notebook erhalte ich die Meldung
...
\ **4.2.13.** Wenn ich FreeBSD auf einem Dell Poweredge XE installiere,
...
\ **4.2.14.** Ich verwende eine Netzwerkkarte vom Typ IBM EtherJet PCI,
...
\ **4.2.15.** Wenn ich w?hrend einer Installation auf einer IBM
