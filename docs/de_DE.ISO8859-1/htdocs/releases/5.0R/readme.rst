...
| $FreeBSD: src/release/doc/de\_DE.ISO8859-1/readme/article.sgml,v 1.10
...
.. raw:: html
    ...
    5.0-CURRENT. Es enth?lt einige Hinweise, wie Sie FreeBSD beziehen
...
5.0-CURRENT und repr?sentiert den aktuellen Zustand im Entwicklungszweig
5-CURRENT.
...
AMD, Cyrix oder NexGen \`\`x86''-basierte PCs, Compaq (ehemals DEC)
Alpha und UltraSPARC Systeme. Versionen f?r die Plattformen IA64 und
PowerPC sind ebenfalls in der Entwicklung. FreeBSD unterst?tzt viele
...
installieren zu k?nnen. Die ?ber 7,800 Ports (vom Editor ?ber
...
Dieser snapshot ist prim?r f?r Bastler und andere Benutzer gedacht, die
sich an der laufenden Entwicklung von FreeBSD beteiligen wollen. Das
Entwicklerteam von FreeBSD bem?ht sich zwar, da? jeder snapshot so
funktioniert wie versprochen, aber 5-CURRENT ist nun einmal der
Entwicklungszweig.

Wenn Sie einen snapshot benutzen wollen, m?ssen Sie sich mit der Technik
von FreeBSD und den aktuellen Vorg?ngen bei der Entwicklung von FreeBSD
5-CURRENT (wie sie in der Mailingliste 'FreeBSD-CURRENT'
``<freebsd-current@FreeBSD.org>`` diskutiert werden) auskennen.

[Anmerkung des ?bersetzers: Auf der Mailingliste 'FreeBSD-CURRENT'
``<freebsd-current@FreeBSD.org>`` wird Englisch gesprochen und die
meisten Dokumente sind ebenfalls in dieser Sprache verfa?t worden. Ohne
gute Englischkenntnisse kommt man also nicht weit. Allerdings ist die
...
Wenn Sie mehr an einem st?rungsfreien Betrieb von FreeBSD als an den
...
werden ausf?hrlich getestet und auf Qualit?t gepr?ft, um Zuverl?ssigkeit
und Betriebssicherheit garantieren zu k?nnen.
...
Wenn Sie die Entwicklung von 5-CURRENT mitverfolgen, *m?ssen* Sie die
Mailingliste 'FreeBSD-CURRENT' ``<freebsd-current@FreeBSD.org>`` lesen.
Nur so k?nnen Sie die teilweise lebensnotwendigen Informationen ?ber die
aktuellen Entwicklungen und ?nderungen erhalten, die Sie f?r den Betrieb
des Systems ben?tigen.
...
``<majordomo@FreeBSD.org>``, die als Text nur das Schl?sselwort ``help``
in einer Zeile enth?lt. Dadurch erhalten Sie weitergehende
Informationen, wie Sie die diversen Mailinglisten abbonieren k?nnen, wie
Sie auf die Archive zugreifen k?nnen, usw. Es gibt viele Mailinglisten,
die nur mit einem ganz bestimmten Thema befassen und hier nicht
aufgef?hrt sind. Weitergehende Informationen finden Sie im Bereich
...
Mailingliste 'FreeBSD problem reports' ``<freebsd-bugs@FreeBSD.org>`` zu
senden.
...
Sie finden weitergehende ?ber diese snapshot Distribution in anderen
...
-  ``EARLY.TXT``: Spezielle Hinweise f?r die ersten Anwender von FreeBSD
   5.0-RELEASE. Wenn dies Ihre erste Begegnung mit FreeBSD 5-CURRENT ist
   und/oder Sie noch eine 5.\ ``X`` Version benutzt haben, sollten Sie
...
-  ``RELNOTES.TXT``: Die begleitenden Informationen zu dieser Release
   ...
   und die ?nderungen seit der letzten Version (FreeBSD 4.0-RELEASE).
...
zur Verf?gung steht (zur Zeit i386 und alpha) finden Sie diese Dokumente
w?hrend der Installation normalerweise unter dem Men?punkt
Dokumentation. Um nach der Installation des Systems dieses Men?
zugreifen zu k?nnen, m?ssen Sie das Programm
`sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+5.0-current>`__
erneut aufrufen.
...
`tuning(7) <http://www.FreeBSD.org/cgi/man.cgi?query=tuning&sektion=7&manpath=FreeBSD+5.0-current>`__
(Hinweise, wie sie die Performance Ihres Systems verbessern k?nnen),
`security(7) <http://www.FreeBSD.org/cgi/man.cgi?query=security&sektion=7&manpath=FreeBSD+5.0-current>`__
(eine Einf?hrung in das Thema Sicherheit unter FreeBSD), und
`style(9) <http://www.FreeBSD.org/cgi/man.cgi?query=style&sektion=9&manpath=FreeBSD+5.0-current>`__
...
aus der ganzen Welt bedanken, ohne die diese snapshot niemals m?glich
gewesen w?re.
