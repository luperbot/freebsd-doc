...
Alpha/AXP Systemen. Es beschreibt die Komponenten von FreeBSD, die in
...
Die Linux-Emulation unterst?tzt jetzt Threads.
...
Der Treiber f?r die Konsole unterst?tzt jetzt TGA-Grafikkarten.
...
FreeBSD wurde erfolgreich auf dem 1 HE gro?en Server API CS20 getestet.

Der AlphaServer 2100A (\`\`Lynx'') wird jetzt unterst?tzt.

Der Kernel wurde erweitert, um auf ?lteren Modellen der Alpha CPU (EV4
und EV5) Befehle der neueren Modelle emulieren zu k?nnen. Damit es ist
m?glich, Programme wie **Adobe Acrobat 4**, deren Quellcode nicht
verf?gbar ist, auf der EV4 und EV5 zu nutzen.

Die SMP-Unterst?tzung auf der Alpha funktioniert jetzt.

Die folgenden Maschinen/Karten werden aus dem bei der Installation
verwendeten Kernel entfernt, damit er wieder auf 1.44 MByte Diskette
pa?t: Multia, NoName, PC64, EB64, Aspen Alpine, sa (SCSI Bandlaufwerke),
amr, parallele Schnitstellen, vx (3c590, 3c595), pcn (AMD Am79C97x PCI
10/100), sf (Adaptec AIC-6915), sis (SiS 900/SiS 7016), ste (Sundance
ST201 (D-Link DFE-550TX)), wb (Winbond W89C840F).

Alpha-Maschinen mit TurboChannel werden nicht mehr unterst?tzt.
...
funktioniert jetzt auch auf der Alpha.
