...
.. raw:: html
    ...
    FreeBSD/pc98 5.2-RELEASE, dabei wird besonderen Wert auf den Bezug
...
Wenn Sie FreeBSD auf einem NEC PC-98x1 System benutzen wollen, mu? das
System mindestens ?ber einen 386 Prozessor (es tut uns ja leid, aber die
286 CPU wird nicht unterst?tzt) und mindestens 5 MByte RAM f?r die
...
?bertragen, dabei handelt es sich entweder um die Disketten-Images
``kern.flp`` und ``mfsroot.flp``, die f?r 1.44 MByte Diskettenlaufwerke
gedacht sind; oder um die Disketten-Images ``kern-small.flp`` und
``mfsroot-small.flp``, die f?r 1.2 MByte Diskettenlaufwerke gedacht
sind.
...
Dazu m?ssen Sie ``rawrite.exe`` und ``kern.flp`` in ein Verzeichnis
...
FreeBSD-CD und ein unterst?tztes CDROM Laufwerk besitzen, k?nnen Sie die
Installation wie folgt beginnen:

-  Verwenden Sie einen Satz FreeBSD Bootdisketten, die notwendigen
   Zutaten finden Sie im bei jeder FreeBSD Distribution im Verzeichnis
   ``floppies/``. Wenn Sie eine DOS-Shell benutzen, k?nnen Sie das
   Script ``makeflp.bat`` verwenden. Weitere Informationen zu diesem
   Thema finden Sie im Kapitel `Abschnitt 1.3 <#FLOPPIES>`__. Danach
   m?ssen Sie nur noch von der ersten Diskette booten, um die
   FreeBSD-Installation zu starten.

Wenn Ihr Rechner kein CDROM Laufwerk hat, k?nnen Sie FreeBSD auch direkt
?ber das Internet installieren, wenn Sie PPP, SLIP oder eine feste
Verbindung haben. In diesem Fall m?ssen Sie ebenfalls die beiden FreeBSD
Bootdisketten aus den Dateien ``floppies/kern.flp`` und
``floppies/mfsroot.flp`` erzeugen, wie in `Abschnitt 1.3 <#FLOPPIES>`__
beschrieben. Booten Sie Ihren Rechner von der Diskette mit ``kern.flp``;
und legen Sie die Diskette mit ``mfsroot.flp`` ein, sobald Sie dazu
aufgefordert werden. Weitere Informationen zu dieser Art der
Installation finden Sie in `Abschnitt 1.5.5 <#FTPNFS>`__.
...
prim?ren DOS-Partition (``A:``) liegen. Wenn Sie zum Beispiel die
...
``A:\FREEBSD`` - die Distribution ``BIN`` ist nur die
Minimalanforderung.
...
1.6. Fragen und Antworten f?r Benutzer von NEC PC-98x1
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
4.2. Typische Probleme bei der Installation auf NEC PC-98x1 Systemen
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
