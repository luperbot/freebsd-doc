...
Treiber unterst?tzt jetzt die Ruhemodi C1-C3 auf Einzel- und
Multiprozessorsystemen, um gem?? ACPI 2.0 Stromverbrauch und
W?rmeentwicklung zu reduzieren. Die Unterst?tzung f?r die Reduzierung
des CPU-Taktes wurde ebenfalls an den ACPI 2.0-Standard angepa?t.

`atkbd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=atkbd&sektion=4&manpath=FreeBSD+5.2-RELEASE>`__
meldete bei der Initialisierung der Console f?lschlicherweise immer eine
AT-Tastatur an, auch wenn keines angeschlossen war. Dieser Fehler wurde
behoben, damit entf?llt das ``kbdcontrol -k /dev/kbd1``, wenn nur eine
USB-Tastatur angeschlossen ist. [MERGED]

Der
`cx(4) <http://www.FreeBSD.org/cgi/man.cgi?query=cx&sektion=4&manpath=FreeBSD+5.2-RELEASE>`__
Treiber f?r serielle Schnitstellen auf Cronyx-Sigma Karten wurde
?berarbeitet. Eine der ?nderungen ist ein neueres Konfigurationsprogramm
mit dem Namen
...
verbessern. Zwei f?r Anwender sichtbare Folgen sind die Nutzbarkeit von
SMP-Kerneln auf UP-Systemen und die SMP-Unterst?tzung im SMP-Kernel
(GENERIC). Zus?tzlich wurde die Kernel-Option options APIC\_IO durch
device apic ersetzt.

Ein Integer-?berlauf, der auf PAE-Systemen mit sehr viel Speicher zu
einer Kernel Panic f?hren konnte, wurde beseitigt.
...
Die logischen CPUs (f?r HyperThreading) werden jetzt gem?? den
Einstellungen im BIOS aktiviert bzw. deaktiviert. In ?lteren Versionen
waren sie immer deaktiviert und mu?ten explizit aktiviert werden.
...
Der Treiber hfa wurde aktualisiert und stellt jetzt die Firmware-Version
4.1.12 sowie eine beschr?nkte Anzahl von CBR Kan?len zur Verf?gung.
...
zur Unterst?tzung der Matsushita CR-562 and CR-563 CDROM-Laufwerke steht
jetzt wieder zur Verf?gung.
...
Die Bibliothek ``libthr`` wird jetzt standardm??ig erzeugt und
installiert. Die Bibliothek stellt die Routinen f?r 1:1 Threading zur
Verf?gung.
