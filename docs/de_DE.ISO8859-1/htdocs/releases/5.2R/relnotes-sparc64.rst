...
Distribution. Sie k?nnen Sie von
ftp://ftp.FreeBSD.org/pub/FreeBSD/development/sparc64/ und allen seinen
...
Es gibt eine neue OFW PCI Infrastruktur, die mit der Kernel-Option
OFW\_NEWPCI aktiviert werden kann. Durch diese neue Infrastruktur werden
PCI-Busse besser unterst?tzt. Eine f?r den Anwender sichtbare ?nderung
betrifft die Nummerierung der Ger?te, sie ?hnelt jetzt mehr der von
Solaris™ verwendeten Nummerierung. Dadurch kann es zu ?nderungen kommen,
wenn mehr als ein Ger?t eines Types vorhanden ist. Im Standardkernel
(``GENERIC``) ist die Option OFW\_NEWPCI bereits aktiviert.
...
Treiber steht jetzt auch auf FreeBSD/sparc64 Systemen zur Verf?gung.
Damit ist es jetzt m?glich, das System ?ber die normale Console zu
installieren und auf Systemen mit einer Creator3D Grafikkarte (wie z.B.
der Ultra 30) sollte jetzt auch **XFree86** laufen.
