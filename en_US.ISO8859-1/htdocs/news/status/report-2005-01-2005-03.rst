...
-  `Common Address Redundancy Protocol -
   CARP <#Common-Address-Redundancy-Protocol---CARP>`__
-  `FreeBSD Java Project <#FreeBSD-Java-Project>`__
-  `FreeBSD Release Engineering <#FreeBSD-Release-Engineering>`__
-  `GELI - GEOM class for providers
   encryption <#GELI---GEOM-class-for-providers-encryption>`__
-  `GSHSEC - GEOM class for handling shared
   secret <#GSHSEC---GEOM-class-for-handling-shared-secret>`__
-  `Secure Updating <#Secure-Updating>`__
...
-  `ATAPI/CAM <#ATAPI/CAM>`__
-  `Coverity Code Analysis <#Coverity-Code-Analysis>`__
-  `cpufreq <#cpufreq>`__
-  `drm <#drm>`__
-  `Filesystem journalling for UFS <#Filesystem-journalling-for-UFS>`__
-  `Infrastructure Cleanup <#Infrastructure-Cleanup>`__
-  `Interrupt Latency <#Interrupt-Latency>`__
-  `Low-overhead performance monitoring for
   FreeBSD <#Low-overhead-performance-monitoring-for-FreeBSD>`__
-  `Many subdirs for UFS <#Many-subdirs-for-UFS>`__
-  `Status Report for FreeBSD ATA driver
   project <#Status-Report-for-FreeBSD-ATA-driver-project>`__
-  `Storage driver SMPng locking <#Storage-driver-SMPng-locking>`__

Network infrastructure
~~~~~~~~~~~~~~~~~~~~~~

-  `Dingo <#Dingo>`__
-  `IPv6 Support for IPFW <#IPv6-Support-for-IPFW>`__
-  `Move ARP out of routing table <#Move-ARP-out-of-routing-table>`__
-  `netgraph(4) status report <#netgraph(4)-status-report>`__
-  `New Wireless Drivers <#New-Wireless-Drivers>`__
-  `Removable interface
   improvements. <#Removable-interface-improvements.>`__
-  `Support for telephone hardware (aka
   Zaptel) <#Support-for-telephone-hardware-(aka-Zaptel)>`__
-  `Wireless Networking Support <#Wireless-Networking-Support>`__

Userland programs
~~~~~~~~~~~~~~~~~

-  `libthread <#libthread>`__
-  `Pipe namespace added to
   portalfs <#Pipe-namespace-added-to-portalfs>`__
...
-  `XenFreeBSD - FreeBSD on Xen <#XenFreeBSD---FreeBSD-on-Xen>`__
...
-  `Update of the Linux userland
   infrastructure <#Update-of-the-Linux-userland-infrastructure>`__

Vendor / 3rd Party Software
~~~~~~~~~~~~~~~~~~~~~~~~~~~

-  `OpenBSD packet filter - pf <#OpenBSD-packet-filter---pf>`__
-  `twa driver <#twa-driver>`__
...
-  `IMUNES - a FreeBSD based kernel-level network topology
   emulator <#IMUNES---a-FreeBSD-based-kernel-level-network-topology%0A----emulator>`__
...
CARP has been committed to HEAD and MFCed to RELENG\_5. It will be
...
Gleb's work was sponsored by `Rambler <http://www.rambler.ru>`__ .

Open tasks:
~~~~~~~~~~~

#. Improve vlan(4) support. Test ng\_eiface(4).
#. Improve locking, consider removing interface layer.
...
#. Volunteers to look into some serious problems with JDK 1.5.0 on
   FreeBSD 4.x
...
and low-risk features to the RELENG\_4 branch for the foreseeable
future.
...
| URL:
  http://perforce.freebsd.org/depotTreeBrowser.cgi?FSPC=//depot/user/pjd/geom%5fclasses/sys/geom/eli&HIDEDEL=NO
|  URL:
  http://perforce.freebsd.org/depotTreeBrowser.cgi?FSPC=//depot/user/pjd/geom%5fclasses/sbin/geom/class/eli&HIDEDEL=NO
...
-  if there is a crypto hardware available, GELI will run cryptography
   on it automatically; if not, it starts dedicated kernel thread and do
   crypto software work in there
...
-  user will be asked for the passphrase before root file system is
   mounted
-  uses "PKCS #5: Password-Based Cryptography Specification Version 2.0"
   for user passphrase protection (optional)
...
-  provider can be configured at attach time to automatically detach on
   last close (so user don't have to remember to detach after unmounting
   file system)
...
URL:
http://www.freebsd.org/cgi/man.cgi?query=gshsec&apropos=0&sektion=0&manpath=FreeBSD+6.0-current&format=html
...
#. Translate the English handbook, then review the Dutch handbook
#. Translate the English FAQ, then review the Dutch FAQ
#. Translate the English Articles, then review the Dutch Articles
...
Thanks to Scott Long and S?ren Schmidt for their participation in the
integration work.
...
URL:
http://www.freebsd.org/cgi/man.cgi?query=cpufreq&manpath=FreeBSD+6.0-current&format=html
...
power\_profile that it should switch settings when the AC line state
...
Current hardware drivers include acpi\_perf (ACPI CPU performance
...
support). Other drivers for relative hardware include acpi\_throttle
...
#. We'd appreciate someone with a Transmeta CPU converting the existing
...
#. Various other architectures, including ARM, have CPU power control
   that could be implemented as a cpufreq driver.
#. The powerd(8) algorithm is rather simple and we'd appreciate more
   help in testing it and alternative algorithms with various workloads.
   The -v flag causes powerd to report frequency transitions and print a
   summary of total energy used upon termination. This should help
   testers profile their algorithms.
...
did the necessary fix to vm\_mmap. New development drivers were added
for mach64 and r300 (see URL for info). The nearly-finished code for
savage and i915 were also added, but left disconnected from the build.
However, the most visible change is likely the support for texture
tiling, color tiling, and HyperZ on Radeons, which (with updated
userland) likely provide a 50-75% framerate increase in many
applications.

Open tasks:
~~~~~~~~~~~

#. Find someone with newbus knowledge to figure out why the i915 won't
   attach to drmsub0.
#. Finish porting the savage driver.
#. Integrate busdma code from Tonnerre (NetBSD).
...
#. On pc98, there's no such thing as an ISA bus. It is desirable to move
   to having cbus appear in the probe messages. This would also allow
   for additional segregation of pc98 specific code in the drivers and
   eliminate many ifdefs. Ideally, isa and cbus would share a common
   newbus ancestor class so their similarities can be exploited (they
   both have PNPBIOS enumeration methods, for example).
#. cbus devices can have complicated resources. There's support for
   ...
   (floppy, ata) a little.
#. The hints mechanism provides a way to associate hardware with drivers
   and resource that would otherwise be completely unknown to the
   system. A refinement in the hints mechanism to allow matching of
   ...
   by acpi or some other enumeration method. Chances are good that these
   seemingly related concepts may need separate implementations due to
   the decision points for unit assignment.
#. Pccard, cardbus and usb probe their devices after interrupts are
   enabled. It would be desirable to hook into new kernel APIs to allow
   the mounting of root to be put off until those systems know that they
   are done with their initial probe of the devices present at boot.
...
#. I need to measure the latencies with 4.x and current to characterize
   ...
   cause.
#. I need to characterize different parts of our ISR routines to see if
   some of the variation I've seen so far can be reduced by improved
   coding techniques.
#. I need to re-run my tests with 5.4 and summarize my results in a
   paper.
...
#. The code needs to be tested on Intel Pentium-M, Celeron, Pentium II
...
URL:
http://groups-beta.google.com/group/muc.lists.freebsd.fs/browse_frm/thread/a36d1143d695287e/40cad00cf2c0823b?hl=en#40cad00cf2c0823b
...
Inc <http://www.freebsdsystems.com>`__ and `ImproWare, AG,
Switzerland <http://www.imp.ch>`__ , the LSI MegaRAID (AMR) and
IBM/Adaptec ServeRAID (IPS) drivers have been locked. SMPng locking is a
key step in improving the performance of system drivers in FreeBSD 5.x
and beyond, and both of these drivers are showing the benefits of this.
FreeBSD 5.4 will contains these improvements when it is released.
...
Lorenzo. I updated it to use PFIL\_HOOKS and fixed a few minor issues.
...
#. Testing.
#. IP6FW to IPFW migration guide.
#. Patches relative to 5-STABLE.
...
#. I am waiting for review feedback from my mentor Andre. I need locking
   experts to help me fix my giant-lock shortcut. I am hoping to send
   out the code for wider review soon.
...
| URL:
  http://www.freebsd.org/cgi/man.cgi?query=ng_netflow&manpath=FreeBSD+6.0-current
|  URL:
  http://www.freebsd.org/cgi/man.cgi?query=ng_ipfw&manpath=FreeBSD+6.0-current
...
ng\_netflow(4) node, which implements NetFlow version 5 accounting of
IPv4 packets. ng\_ipfw(4) node, which diverts packets from ipfw(4) to
netgraph(4) and back. A well known ng\_ipacct node has been added to
ports tree.

SMP. Nodes, which need to allocate unique names have been protected with
mutex in RELENG\_5, and subr\_unit allocator in HEAD. Nodes, which need
to run periodical jobs were reworked to use mpsafe ng\_callout() API.
ng\_tty(4) node has been overhauled to be compatible with
debug.mpsafenet=1. NetGraph ISR and callout are now declared MPSAFE in
HEAD.

NetGraph flow control. Two nodes ng\_ether(4) and ng\_cisco(4) have been
...
ng\_one2many(4) node - listening to these flow control messages from
downstream.

Open tasks:
~~~~~~~~~~~

#. more SMP testing of many nodes
#. review locking of graph restructuring
#. ng\_nat node - an in-kernel natd(8)
#. make ng\_bridge(4) multithreaded
...
|  *iwi* : driver for Intel PRO/Wireless 2200BG/2225BG/2915ABG adapters
  (PCI or MiniPCI).
...
#. Create ports for ipw and iwi firmwares.
#. Add IBSS support to iwi.
#. Add WPA (802.11i) support to ipw and iwi.
#. Add hardware encryption (WEP, TKIP and CCMP) support in ral and ural.
#. Add automatic rate adaptation support to ural.
...
tree shortly and will then add a new function if\_alloc() that will
allocate struct ifnets. if\_detach() will be modified to destroy them.
...
#. More testing of PRI/BRI drivers.
#. Add support for channelized DS3 card(s).
...
#. Drivers other than ath and ndis need updates to support the new
   security protocols.
#. hostapd needs work to support the IAPP and 802.11i preauthentication
   protocols (these are simple conversions of existing Linux code).
#. The OpenBSD dhclient program has been ported but needs a developer
...
libthread\_db was updated to support debugging the new libthr. It is an
...
#. I am working on thread creation performance, currently it needs
   ...
   signal mask and PTHREAD\_SCOPE\_PROCESS/SYSTEM; in future maybe even
   CPU affinity masks, when userland entry code is executed, the thread
   is already fully setup.
#. Process shareable synchronization objects. In Current FreeBSD does
...
#. Figuring out why some small packets transmit with error
...
#. FreeBSD support for running as Domain 0, i.e. running as the hosting
   operating system.
#. FreeBSD support for VM checkpoint and migration.
...
`FreshSource <http://www.freshsource.org/>`__ uses the same database as
FreshPorts but has a wider reporting scope. In recent months, FreshPorts
has been enhanced to process and include
`VuXML <http://www.vuxml.org/freebsd/>`__ information. In addition,
RESTRICTED and NO\_CDROM have been added to list of things that
FreshPorts keeps track of. For unmaintained ports, we recently added
this message:
...
#. Provide a copy/paste method for updating watch lists
#. improvement of query times for "People watching this port, also
   watch"
#. pagination of commits within a port
#. pagination of watch lists
#. create an RSS feed for individual watch lists
...
#. Further progress has been made in cracking down on ports that install
   files outside the approved directories and/or do not deinstall
   cleanly (see "Extra files not listed in PLIST" on
   `pointyhat <http://pointyhat.freebsd.org/errorlogs/>`__ ) and this
   will remain a focus area. We appreciate everyone who has sent in PRs
   or committed fixes.
#. Demand for new features and revisions for bsd.port.mk is still very
   high and the portmgr team is trying to work through them all.
#. We still have a large number of PRs that have been assigned to
   committers for some time (in fact, they constitute the majority). One
   goal of portmgr in the coming months is to try to reduce this number,
   and we would like to ask our committers to help us out as much as
   possible.
...
#. Refactoring the common RPM code into bsd.rpm.mk.
#. Determining which up-to-date Linux distribution to use as the next
...
OpenBSD is about to release `version
3.7 <http://www.openbsd.org/37.html>`__ . There are
`patches <http://people.freebsd.org/~mlaier/pf37/>`__ available to catch
up with the development done in OpenBSD 3.6 and 3.7. These patches are
in an early stage, but ready for testing, please help.

Otherwise there was not much activity on pf, as it already is quite
stable. Other work, such as CARP and if\_bridge are having impact on pf
in FreeBSD however, please see the respective reports.

Open tasks:
~~~~~~~~~~~

#. Alpha/Betatesting of the 3.7 import
#. Testing with if\_bridge
...
Contact: Vinod Kashyap <`vkashyap at
amcc.com <mailto:vkashyap%20at%20amcc.com>`__\ >

A newly re-architected twa(4) driver was committed to 6 -CURRENT on
04/12/2005. Highlights of this release are:

#. The driver has been re-architected to use a "Common Layer" (all
   tw\_cl\* files), which is a consolidation of all OS-independent parts
   of the driver. The FreeBSD OS specific portions of the driver go into
   an "OS Layer" (all tw\_osl\* files). This re-architecture is to
   ...
   there's interest in porting the 3ware driver to any other OS, you may
   contact ctchu at amcc.com to get a copy of the CLPI specifications.
#. The driver takes advantage of multiple processors. It does not need
   to be Giant protected anymore.
#. The driver has a new firmware image bundled, the new features of
...
BSDCan made a strong debut in `2004 <http://www.bsdcan.org/2004/>`__ .
The favorable reception gave us a strong incentive for
`2005 <http://www.bsdcan.org/2005/>`__ . We have been rewarded with a
very interesting `program <http://www.bsdcan.org/2005/schedule.php>`__
and a higher rate of registrations. Percentage-wise, we have more
Europeans than last year as they have decided that the trip across the
Atlantic is worth taking. We know they won't be disappointed. See you at
BSDCan 2005!
...
Security Officer, joining Dag-Erling Sm?rgrav in that position. The
...
In the past months both the `VuXML web
site <http://vuxml.FreeBSD.org/>`__ and the
`FreshPorts <http://www.FreshPorts.org/>`__ VuXML integration have been
improved. The VuXML web site has had a face lift and, among other
things, each package now has a separate web page which lists all
documented vulnerabilities for the particular package.
`CVE <http://cve.mitre.org/>`__ information is now also included
directly on the VuXML web site.
