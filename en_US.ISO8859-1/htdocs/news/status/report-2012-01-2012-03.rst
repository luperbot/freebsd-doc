...
-  `FreeBSD Services Control <#FreeBSD-Services-Control>`__
-  `GNU-Free C++11 Stack <#GNU-Free-C++11-Stack>`__
-  `Growing filesystems online <#Growing-filesystems-online>`__
-  `The FreeNAS Project <#The-FreeNAS-Project>`__
...
-  `The bsdconfig(8) utility <#The-bsdconfig(8)-utility>`__
...
-  `The FreeBSD Foundation Team
   Report <#The-FreeBSD-Foundation-Team-Report>`__

Kernel
~~~~~~

-  `DTrace Probes for the
   linuxulator <#DTrace-Probes-for-the-linuxulator>`__
-  `HDMI/DisplayPort Audio Support in HDA Sound Driver
   (snd\_hda) <#HDMI/DisplayPort-Audio-Support-in-HDA-Sound-Driver%0A------(snd_hda)>`__
-  `Improved hwpmc(9) Support for
   MIPS <#Improved-hwpmc(9)-Support-for-MIPS>`__
-  `isci(4) SAS Driver <#isci(4)-SAS-Driver>`__

Network Infrastructure
~~~~~~~~~~~~~~~~~~~~~~

-  `Atheros 802.11n Support <#Atheros-802.11n-Support>`__
-  `IPv6 Performance Analysis <#IPv6-Performance-Analysis>`__
-  `Multi-FIB: IPv6 Support and Other
   Enhancements <#Multi-FIB:-IPv6-Support-and-Other-Enhancements>`__
...
-  `FreeBSD/arm on Various TI
   Boards <#FreeBSD/arm-on-Various-TI-Boards>`__
-  `FreeBSD/powerpc on Freescale QorIQ
   DPAA <#FreeBSD/powerpc-on-Freescale-QorIQ-DPAA>`__
-  `NAND File System, NAND Flash Framework, NAND
   Simulator <#NAND-File-System,-NAND-Flash-Framework,-NAND-Simulator>`__
-  `Porting DTrace to MIPS and ARM <#Porting-DTrace-to-MIPS-and-ARM>`__

Ports
~~~~~

-  `A New linux\_base Port Based Upon
   CentOS <#A-New-linux_base-Port-Based-Upon-CentOS>`__
-  `BSD-licensed sort Utility (GNU sort
   Replacement) <#BSD-licensed-sort-Utility-(GNU-sort-Replacement)>`__
-  `KDE/FreeBSD <#KDE/FreeBSD>`__
-  `Perl Ports Testing <#Perl-Ports-Testing>`__
-  `The FreeBSD Haskell Ports <#The-FreeBSD-Haskell-Ports>`__
-  `The FreeBSD Ports Collection <#The-FreeBSD-Ports-Collection>`__
...
We still need a replacement for some parts of libgcc\_s and for the
linker, but we're on track for a BSD licensed C++ stack in 10.0.

Open tasks:
~~~~~~~~~~~

#. Test ports with libc++. Hopefully most will Just Work, but others may
   need patches or have a hard dependency on libstdc++.
#. Enable building libc++ by default. This is dependent upon building
...
#. The write suspension infrastructure (/dev/ufssuspend) implemented to
   make resizing possible makes it also possible to implement online
   tunefs(8) and fsck(8).
#. Right now, there is no way for a GEOM class to veto resizing —
   classes are notified about resize and they can either adapt, or
   wither. Many classes store their metadata in the last sector, though,
   so resizing a partition containing e.g. gmirror will make it
   inoperable. It would be nice if geom\_mirror(4) could veto resizing,
...
As of r231057, there is a WITH\_CLANG\_EXTRAS option for src.conf(5),
...
Also, as of r232322, there is a WITH\_CLANG\_IS\_CC option for
src.conf(5), which will install Clang as /usr/bin/cc, /usr/bin/c++ and
/usr/bin/cpp, making it the default system compiler. Unless you also use
the WITHOUT\_GCC option, gcc will still be available as /usr/bin/gcc,
...
#. One of the most important tasks at the moment is to actually build
...
#. Clang should have gotten better support for cross-compiling after
   3.0, so as soon as a 3.1 version is imported, we will need to look at
   ways to get the FreeBSD world and kernels to cross-compile. This is
   mainly of use for ARM and MIPS, which are architectures you usually
   do not want to build natively on.
#. Help to make unwilling ports build with Clang is always needed, and
   greatly appreciated. Please mail the maintainer of your favorite port
   with patches, or file PRs.
...
REG\_ICASE flag are not yet covered. This is the next major step to
...
|  URL:
  http://druidbsd.sf.net/download/bsdconfig/bsdconfig-20120512-1.svg
|  URL:
...
We are pleased to
`announce <http://www.FreeBSDfoundation.org/announcements.shtml>`__ the
addition of George Neville-Neil to our board of directors. Deb Goodkin,
our Director of Operations, was `interviewed by
bsdtalk <http://bsdtalk.blogspot.com/>`__.

We announced a call for project proposals. We will accept proposals
until April 30th. Please read `Project Proposal
Procedures <http://www.FreeBSDfoundation.org/documents%20/FreeBSD%20Foundation%20Proposals%20March%202012.pdf>`__
to find out more.
...
HDMI/DisplayPort Audio Support in HDA Sound Driver (snd\_hda)
-------------------------------------------------------------

Contact: Alexander Motin <mav@FreeBSD.org\ >

snd\_hda(4) driver got number of improvements to better support
...
-  Added support for more multichannel formats. For HDMI and DisplayPort
   device now supported: 2.0, 2.1, 3.0, 3.1, 4.0, 4.1, 5.0, 5.1, 6.0,
   6.1, 7.0 and 7.1 channels.
-  Added support for compressed streams passthrough with data rate 6.144
   - 24Mbps, such as DTS-HD Master Audio or Dolby TrueHD.
...
#. Make better use of received EDID-Like Data.
#. Identify and fix problem with older NVIDIA cards.
...
#. Fix up hostap side power save handling.
#. Implement filtered frames support in the driver.
#. Fix scan/bgscan to correctly buffer and retransmit frames when going
   off channel, so frames are not just "dropped" - this causes issues in
   the aggregation sessions and may cause traffic stalls.
#. Test/fix any issues with adhoc 802.11n support.
...
#. Further translation work of outdated documents in both
...
#. Completing missing peripherals: DMA, SPI, MMC/SD, Video, Audio.
#. Completing SMP support and testing.
#. Importing BeagleBoard (OMAP3) code to SVN.
...
|  URL:
  http://www.freescale.com/webapp/sps/site/homepage.jsp?code=64BIT&fsrch=1&sr=1
...
#. Userland DTrace support for MIPS.
#. Investigate amount of effort required for getting fbt provider work
   at least partially.
#. Find proper solution for cross-platform CTF data generation (required
   for ARM).

--------------

A New linux\_base Port Based Upon CentOS
----------------------------------------

Contact: Alexander Leidinger <netchild@FreeBSD.org\ >

We got a PR with a linux\_based port which is based upon CentOS 6.
...
#. Repocopy by portmgr.
#. Add conflicts in other linux\_base ports.
#. Commit the CentOS based one.
#. Some cleanup.
...
#. Add BSD sort into HEAD as an alternative, installed as bsdsort. If
   proven to work as expected, change it to the default sort version and
   remove GNU sort.
#. Investigate the possibility of a multi-threaded sort implementation
   and implement it, if it proves more efficient.
#. Upgrade BSD sort features to include some obscure new features in the
   latest GNU sort version 8.15.
...
#. Testing KDE SC 4.8.2.
#. Testing KDE PIM 4.8.2.
...
#. Finish work on patch to bring testing support to ports.
#. Add additional support for testing other types of ports such as
   python and ruby.
...
#. Looking for help getting ports to build with clang.
#. Looking for help with Tier-2 architectures.
#. ports broken by src changes.
#. ports failing on pointyhat.
#. ports failing on pointyhat-west.
#. ports that are marked as BROKEN.
