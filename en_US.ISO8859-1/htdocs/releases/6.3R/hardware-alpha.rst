...
2.3.2 `Personal Workstation (“Miata”) <#AEN187>`__
2.3.3 `Evaluation Board 64 family <#AEN297>`__
2.3.4 `Evaluation Board 164 (“EB164, PC164, PC164LX, PC164SX”)
family <#AEN352>`__
2.3.5 `AlphaStation 200 (“Mustang”) and 400 (“Avanti”)
series <#AEN421>`__
2.3.6 `AlphaStation 500 and 600 (“Alcor” & “Maverick” for EV5, “Bret”
for EV56) <#AEN488>`__
2.3.7 `AlphaServer 1000 (“Mikasa”), 1000A (“Noritake”) and
800(“Corelle”) <#AEN600>`__
2.3.8 `DS10/VS10/XP900 (“Webbrick”) / XP1000 (“Monet”) / DS10L
(“Slate”) <#AEN660>`__
2.3.9 `DS20/DS20E (“Goldrush”) <#AEN811>`__
2.3.10 `AlphaPC 264DP / UP2000 <#AEN881>`__
2.3.11 `AlphaServer 2000 (“DemiSable”), 2100 (“Sable”), 2100A
(“Lynx”) <#AEN927>`__
2.3.12 `AlphaServer 4x00 (“Rawhide”) <#AEN998>`__
2.3.13 `AlphaServer 1200 (“Tincup”) and AlphaStation 1200
(“DaVinci”) <#AEN1031>`__
2.3.14 `Alpha Processor Inc. UP1000 <#AEN1061>`__
2.3.15 `Alpha Processor Inc. UP1100 <#AEN1110>`__
2.3.16 `Alpha Processor Inc. CS20, Compaq DS20L <#AEN1158>`__
2.3.17 `Compaq AlphaServer ES40 (“Clipper”) <#AEN1205>`__
2.4 `Supported Hardware Overview <#AEN1242>`__
2.5 `Further reading <#AEN1273>`__
2.6 `Acknowledgments <#AEN1277>`__
...
6.3-RELEASE on the Alpha/AXP hardware platform (also referred to as
FreeBSD/alpha 6.3-RELEASE). It lists devices known to work on this
...
Although you could put the SRM console firmware on a whitebox, OpenVMS
and Digital Unix will refuse to boot on them. FreeBSD in
post-4.0-RELEASE will run on both the white and the blue-box variants.
Before someone asks: the white ones had a rather different (read:
cheaper) Digital price tag.
...
desirable (read: faster / more modern). Spending money on anything less
than an EV5 is not recommended. Hardware available for free is a
different matter of course.

For memory you want at least 32 Mbytes. I have had FreeBSD run on a 16
Mbyte system years ago but you will not enjoy that. Kernel build times
halved when I went to 32 Mbytes. Note that the SRM console steals 2Mbyte
from the total system memory (and keeps it). For more serious work 128
Mbytes or more are recommended. Although Alpha machines typically can
accommodate large to very large physical memory sizes, FreeBSD is
limited to 1 or 2 Gbytes (dependent on the core chipset) of RAM. This
restriction is due to the current implementation of the VM system.
...
(the trailing “a”). Systems designated from day 1 to run Tru64 Unix or
...
about. Cards that are “known good” to the SRM are allowed to be used in
the 64bit slots.
...
the bridge, in one of the 64 bit PCI slots. Graphics performance using a
...
are used by OEM manufacturers/assembly shops. Samsung did its own
...
layout. So, a bank of 4 DIMMs is *not* 4 physically adjacent DIMMs. Note
that the DIMMs are *not* SDRAM DIMMs.
...
The main difference between AS1000 and AS1000A is that AS1000A has 7 PCI
slots whereas AS1000 only has 3 PCI slots and has EISA slots instead.

The AlphaServer 800 system has a smart power controller. This means that
parts of the system remain powered when it is switched off (like an
ATX-style PC power supply). Before servicing the machine remove the
power cord.
...
AlphaServer 800 with an EV5/400 MHz CPU was later re-branded to become a
“DIGITAL Server 3300[R]”, AlphaServer 800 with an EV5/500 MHz CPU was
...
different DIMM sizes are used. Max memory size is 2GB. Note that it uses
buffered EDO DIMMs.
...
.. raw:: html
    ...
    **Note:** Contrary to expectation there is no ``cpu EV6`` defined
    ...
    mandatory to keep
    `config(8) <http://www.FreeBSD.org/cgi/man.cgi?query=config&sektion=8&manpath=FreeBSD+6.3-RELEASE>`__
    happy.
...
shelf. It can either be rackmounted or floorstanding.

The system has a smart power controller. This means that parts of the
system remain powered when it is switched off (like an ATX-style PC
power supply). Before servicing the machine remove all power cord(s).
...
The embedded Adaptec SCSI on the DS20 is disabled and is therefore not
usable under FreeBSD. On the DS20E with recent SRM console code you can
use them with FreeBSD.
...
DIMM slots should be filled “in order” so after bank 0 install in bank 1
and so on.
...
slots. PCI0 also has a PCI/EISA bridge that drives things like the
serial and parallel ports, keyboard/mouse etc. PCI1 has 4 PCI slots and
a Symbios 810 SCSI chip. VGA console cards must be installed in a slot
connected to PCI0.
...
system's physical memory map will contain “holes”. Supported DIMM sizes
are 64 Mbytes and 256 Mbytes. The DIMMs are 72 bit SDRAM based, as the
system employs ECC.
...
`ata(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ata&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
`de(4) <http://www.FreeBSD.org/cgi/man.cgi?query=de&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
(older driver) or
`dc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=dc&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
(newer driver). Some new SRM versions are known to recognize the Intel
8255x Ethernet chips as driven by the FreeBSD
`fxp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fxp&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
TGA support in FreeBSD is not as robust as it should be. In case of
problems it is advisable to try either a serial console or a plain VGA
card.
...
An interesting overview of the history of the Alpha CPU can be found at
...
1.282.2.17 2007/09/13 16:25:43 brueffer Exp $
...
The
`ahc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ahc&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
`amr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=amr&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver include:
...
`isp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=isp&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver include:
...
The
`mfi(4) <http://www.FreeBSD.org/cgi/man.cgi?query=mfi&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
Controllers supported by the
`mlx(4) <http://www.FreeBSD.org/cgi/man.cgi?query=mlx&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver include:
...
The
`ncr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ncr&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`sym(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sym&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
`cd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=cd&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__).
...
-  ATAPI IDE interface
   (`acd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=acd&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__)
...
The
`hme(4) <http://www.FreeBSD.org/cgi/man.cgi?query=hme&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`nge(4) <http://www.FreeBSD.org/cgi/man.cgi?query=nge&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`pcn(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pcn&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`re(4) <http://www.FreeBSD.org/cgi/man.cgi?query=re&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
Adapters supported by the
`rl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=rl&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver include:
...
Adapters supported by the
`sf(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sf&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver include:
...
The
`sis(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sis&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
Adapters supported by the
`sk(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sk&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver include:
...
The
`ste(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ste&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`tl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=tl&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`txp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=txp&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`vr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=vr&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`wb(4) <http://www.FreeBSD.org/cgi/man.cgi?query=wb&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`xl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=xl&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver supports the following hardware:
...
DEC DEFPA PCI
(`fpa(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fpa&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)
...
(`ipw(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ipw&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)

[i386, amd64] Intel PRO/Wireless 2200BG/2915ABG MiniPCI and 2225BG PCI
network adapters
(`iwi(4) <http://www.FreeBSD.org/cgi/man.cgi?query=iwi&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)
...
(`sio(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sio&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)

The
`uart(4) <http://www.FreeBSD.org/cgi/man.cgi?query=uart&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
(`rp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=rp&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)
...
The
`snd\_maestro(4) <http://www.FreeBSD.org/cgi/man.cgi?query=snd_maestro&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`snd\_maestro3(4) <http://www.FreeBSD.org/cgi/man.cgi?query=snd_maestro3&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`snd\_sbc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=snd_sbc&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`ohci(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ohci&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`uhci(4) <http://www.FreeBSD.org/cgi/man.cgi?query=uhci&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
Keyboards
(`ukbd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ukbd&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)

Mice
(`ums(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ums&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)

The
`ulpt(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ulpt&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`umct(4) <http://www.FreeBSD.org/cgi/man.cgi?query=umct&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
The
`umass(4) <http://www.FreeBSD.org/cgi/man.cgi?query=umass&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
...
Floppy drives
(`fdc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fdc&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)

VGA-compatible video cards
(`vga(4) <http://www.FreeBSD.org/cgi/man.cgi?query=vga&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)
...
-  AT-style keyboards
   (`atkbd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=atkbd&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
   driver)
...
-  PS/2 mice and compatible devices, including many laptop pointing
   devices
   (`psm(4) <http://www.FreeBSD.org/cgi/man.cgi?query=psm&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
   driver)
...
.. raw:: html
    ...
    **Note:**
    `moused(8) <http://www.FreeBSD.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+6.3-RELEASE>`__
...
(`ppc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ppc&sektion=4&manpath=FreeBSD+6.3-RELEASE>`__
driver)
