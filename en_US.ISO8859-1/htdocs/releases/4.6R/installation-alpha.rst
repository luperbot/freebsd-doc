...
.. raw:: html
    ...
    FreeBSD/alpha 4.6-RELEASE, with particular emphasis given to
...
Your root filesystem MUST be the first partition (partition ``a``) on
the disk to be bootable.
...
you have a 2.88MB or LS-120 floppy capable of taking a 2.88MB image on
an x86 machine, you may wish to use the single (but twice as large)
``boot.flp`` image. It contains the contents of ``kern.flp`` and
``mfsroot.flp`` on a single floppy. This file should also be used as the
boot file for those mastering \`\`El Torito'' bootable CD images. See
the
`mkisofs(8) <http://www.FreeBSD.org/cgi/man.cgi?query=mkisofs&sektion=8&manpath=FreeBSD+4.6-RELEASE>`__
...
the files ``floppies/kern.flp`` and ``floppies/mfsroot.flp`` using the
instructions found in `Section 1.3 <#FLOPPIES>`__. From the SRM console
prompt (``>>>``), just insert the ``kern.flp`` floppy and type the
following command to start the installation:

.. code:: SCREEN

        >>>boot dva0

Insert the ``mfsroot.flp`` floppy when prompted and you will end up at
...
three disks attached to the machine. The first is a CDROM called
``dka0`` and the other two are disks and are called ``dkc0`` and
``dkc100`` repectively.
...
1.6.1. `Can I boot from the ARC or Alpha BIOS Console? <#Q1.6.1.>`__
1.6.2. `Help! I have no space! Do I need to delete everything
first? <#Q1.6.2.>`__
1.6.3. `Can I mount my Compaq Tru64 or VMS extended
partitions? <#Q1.6.3.>`__
1.6.4. `What about support for Compaq Tru64 (OSF/1)
binaries? <#Q1.6.4.>`__
1.6.5. `What about support for Linux binaries? <#Q1.6.5.>`__
1.6.6. `What about support for NT Alpha binaries? <#Q1.6.6.>`__
...
****\ No. FreeBSD, like Compaq Tru64 and VMS, will only boot from the
SRM console.
...
****\ FreeBSD is not able to run NT applications natively, although it
has the ability to mount NT partitions.
