...
.. raw:: html
    ...
    FreeBSD/i386 5.5-RELEASE, with particular emphasis given to
...
obtaining the FreeBSD 5.5-RELEASE distribution and to beginning the
...
run (although FreeBSD can run on 386 processors with a custom kernel)
and at least 24 MB of RAM. You will need at least 150MB of free hard
...
actual floppies from the ``floppies/`` directory are the ``boot.flp``
and ``kernX``.flp images (for 1.44MB floppies).

Getting these images over the network is easy. Simply fetch the
``release``\ ``/floppies/boot.flp``, and all of the
``release``\ ``/floppies/kernX``.flp files from ftp://ftp.FreeBSD.org/
...
Get several blank, freshly formatted floppies and image copy
``boot.flp`` onto one and the ``kernX``.flp files onto the others. These
images are *not* DOS files. You cannot simply copy them to a DOS or UFS
floppy as regular files, you need to \`\`image'' copy them to the floppy
with ``fdimage.exe`` under DOS (see the ``tools`` directory on your
CDROM or FreeBSD FTP mirror) or the
`dd(1) <http://www.FreeBSD.org/cgi/man.cgi?query=dd&sektion=1&manpath=FreeBSD+5.5-RELEASE>`__
command in UNIX.
...
Assuming that you'd copied ``fdimage.exe`` and ``boot.flp`` into a
directory somewhere. You would do the same for the ``kernX``.flp files,
of course.
...
-  If your system supports the \`\`CDBOOT'' standard for bootable CDROM
   media and you have booting from CD enabled, simply put the FreeBSD
...
-  Build a set of FreeBSD boot floppies from the ``floppies/`` directory
   in every FreeBSD distribution. Read `Section 1.3 <#FLOPPIES>`__ for
   more information on creating the bootable floppies under different
...
``floppies/boot.flp`` and ``floppies/kernX``.flp using the instructions
found in `Section 1.3 <#FLOPPIES>`__. Restart your computer using the
``boot.flp`` disk; when prompted, insert the other disks as required.
Then, please go to `Section 1.5.5 <#FTPNFS>`__ for additional tips on
...
-  If you would be able to FTP install FreeBSD directly from the CDROM
   ...
   password file (using the
   `vipw(8) <http://www.FreeBSD.org/cgi/man.cgi?query=vipw&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
   command):
...
-  If you would rather use NFS to export the CDROM directly to the
   ...
   `rc.conf(5) <http://www.FreeBSD.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+5.5-RELEASE>`__
...
`disklabel(8) <http://www.FreeBSD.org/cgi/man.cgi?query=disklabel&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
and
`newfs(8) <http://www.FreeBSD.org/cgi/man.cgi?query=newfs&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
...
`tar(1) <http://www.FreeBSD.org/cgi/man.cgi?query=tar&sektion=1&manpath=FreeBSD+5.5-RELEASE>`__
...
generally the default for Sun and Linux workstations), you may need to
...
#. If you don't get any output on your serial console, plug the keyboard
   in again. If you are booting from the CDROM, proceed to `step
   5 <#HITSPACE>`__ as soon as you hear the beep.

#. If booting from floppies, when access to the disk stops, insert the
   first of the ``kernX``.flp disks and press **Enter**. When access to
   this disk finishes, insert the next ``kernX``.flp disk and press
   **Enter**, and repeat until all ``kernX``.flp disks have been
...
1.6.5. `Can I run Microsoft® Windows® applications under
FreeBSD? <#Q1.6.5.>`__
1.6.6. `Can I run other Operating Systems under FreeBSD? <#Q1.6.6.>`__
...
**FIPS** does not work on extended DOS partitions. Windows 95/98/ME
FAT32 primary partitions are supported.
...
`doscmd(1) <http://www.FreeBSD.org/cgi/man.cgi?query=doscmd&sektion=1&manpath=FreeBSD+5.5-RELEASE>`__
...
port/package can be found in the FreeBSD Ports Collection. If you're
interested in working on this, please send mail to the
...
****\ There are several ports/packages in the FreeBSD Ports Collection
which can enable the use of many Windows applications. The
```emulators/wine`` <http://www.FreeBSD.org/cgi/url.cgi?ports/emulators/wine/pkg-descr>`__
port/package provides a compatibility layer on top of FreeBSD which
allow many Windows applications to be run within X Windows (XFree86).
...
****\ Again, there are several ports/packages in the FreeBSD Ports
Collection which simulate "virtual machines" and allow other operating
systems to run on top of FreeBSD. The
```emulators/bochs`` <http://www.FreeBSD.org/cgi/url.cgi?ports/emulators/bochs/pkg-descr>`__
port/package allows Microsoft Windows, Linux and even other copies of
FreeBSD to be run within a window on the FreeBSD desktop. The
...
ports/packages allow the commercial VMware virtual machine software to
be run on FreeBSD.
...
architectures, the installation CDROM also contains a \`\`live
filesystem'' in addition to the distribution files. The live filesystem
is useful when repairing or troubleshooting an existing FreeBSD
installation (see `Section 4 <#TROUBLE>`__ for how to use this).

The ``floppies`` directory will be of particular interest to users who
are unable to boot from the CDROM media (but are able to read the CDROM
by other means). It is easy to generate a set of 1.44MB boot floppies
from the ``floppies`` directory (see `Section 1.3 <#FLOPPIES>`__ for
instructions on how to do this) and use these to start an installation
from CDROM, FTP, or NFS. The rest of the data needed during the
installation will be obtained automatically based on your selections. If
you've never installed FreeBSD before, you also want to read the
...
#. The ``base``, ``catpages``, ``crypto``, ``dict``, ``doc``, ``games``,
...
#. The ``packages`` and ``ports`` directories contain the FreeBSD
   ...
   requires about 190MB unpacked. More information on the ports
...
`mtree(8) <http://www.FreeBSD.org/cgi/man.cgi?query=mtree&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
program to verify the installation permissions and checksums against any
possible modifications to the file. When used with the ``base``
...
.. raw:: html
    ...
    **Warning:** Binary upgrades to FreeBSD 5.5-RELEASE from FreeBSD
    4-STABLE are not supported at this time. There are some files
    present in a FreeBSD 4-STABLE whose presence can be disruptive, but
    are not removed by a binary upgrade. One notable example is that an
    old ``/usr/include/g++`` directory will cause C++ programs to
    compile incorrectly (or not at all).

    These upgrade instructions are provided for the use of users
    upgrading from relatively recent FreeBSD 5.5-STABLE snapshots.
...
selected. As a general rule, the ``base`` distribution should be
selected for an update, and the ``man`` distribution if manpages are
already installed. Other distributions may be selected beyond those
originally installed if the administrator wishes to add additional
functionality.
...
a few more steps. In addition to the ``boot.flp`` and ``kernX``.flp
disks required for installation, create the ``fixit.flp`` floppy disk,
in the same way as the other floppy disks. Follow the instructions for
booting the installation program from floppy disk until reaching the
main
`sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
menu. At that point, choose the \`\`fixit'' item from the main
`sysinstall(8) <http://www.FreeBSD.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
menu. Then select the \`\`Floppy'' option from the fixit menu, and
insert the ``fixit.flp`` floppy disk when prompted to do so.
...
in ``/stand`` or ``/mnt2/stand``. In the CDROM version of fixit, these
same programs can be found in ``/stand`` or ``/mnt2/rescue`` (as well as
the rest of the programs from the live filesystem, which can be found
under ``/mnt``).
...
4.2.1. `My system hangs while probing hardware during boot, or it
behaves strangely during install, or the floppy drive isn't
probed. <#Q4.2.1.>`__
4.2.3. `My legacy ISA device used to be recognized in previous versions
of FreeBSD, but now it's not. What happened? <#Q4.2.3.>`__
4.2.4. `I go to boot from the hard disk for the first time after
installing FreeBSD, the kernel loads and probes my hardware, but stops
with messages like: <#Q4.2.4.>`__
4.2.5. `I go to boot from the hard disk for the first time after
installing FreeBSD, but the Boot Manager prompt just prints ``F?`` at
the boot menu each time but the boot won't go any further. <#Q4.2.5.>`__
4.2.6. `The mcd(4) driver keeps thinking that it has found a device and
this stops my Intel EtherExpress card from working. <#Q4.2.6.>`__
4.2.7. `The system finds my ed(4) network card, but I keep getting
device timeout errors. <#Q4.2.7.>`__
4.2.8. `I booted the install floppy on my IBM ThinkPad (tm) laptop, and
the keyboard is all messed up. <#Q4.2.8.>`__
4.2.9. `My system can not find my Intel EtherExpress 16
card. <#Q4.2.9.>`__
4.2.10. `When installing on an EISA HP Netserver, my on-board AIC-7xxx
SCSI controller isn't detected. <#Q4.2.10.>`__
4.2.11. `I have a Panasonic AL-N1 or Rios Chandler Pentium machine and I
find that the system hangs before ever getting into the installation
now. <#Q4.2.11.>`__
4.2.12. `I have this CMD640 IDE controller that is said to be
broken. <#Q4.2.12.>`__
4.2.13. `On a Compaq Aero notebook, I get the message \`\`No floppy
devices found! Please check ...'' when trying to install from
floppy. <#Q4.2.13.>`__
4.2.14. `When installing on a Dell Poweredge XE, Dell proprietary RAID
controller DSA (Dell SCSI Array) isn't recognized. <#Q4.2.14.>`__
4.2.15. `I have an IBM EtherJet PCI card, it is detected by the fxp(4)
driver correctly, but the lights on the card don't come on and it
doesn't connect to the network. <#Q4.2.15.>`__
4.2.16. `When I configure the network during installation on an IBM
Netfinity 3500, the system freezes. <#Q4.2.16.>`__
4.2.17. `When I install onto a drive managed by a Mylex PCI RAID
...
\ **4.2.1.** My system hangs while probing hardware during boot, or it
behaves strangely during install, or the floppy drive isn't probed.
...
****\ FreeBSD 5.0 and above makes extensive use of the system ACPI
service on the i386, amd64 and ia64 platforms to aid in system
configuration if it's detected during boot. Unfortunately, some bugs
still exist in both the ACPI driver and within system motherboards and
BIOS. The use of ACPI can be disabled by setting the
...
information about the boot loader can be found in the FreeBSD Handbook.
...
\ **4.2.3.** My legacy ISA device used to be recognized in previous
versions of FreeBSD, but now it's not. What happened?
...
lack of maintainership or other reasons. Others still exist but are
disabled because of their intrusive hardware probe routines. The
following ISA device drivers fall into this category and can re-enabled
from the third stage boot loader: aha, ahv, aic, bt, ed, cs, sn, ie, fe,
le, and lnc. To do this, stop the loader during it's 10 second countdown
...
\ **4.2.6.** The mcd(4) driver keeps thinking that it has found a device
...
\ **4.2.8.** I booted the install floppy on my IBM ThinkPad (tm) laptop,
...
\ **4.2.10.** When installing on an EISA HP Netserver, my on-board
...
Refer to the FAQ topic 5.3 for an explanation of the problem, and for
...
\ **4.2.11.** I have a Panasonic AL-N1 or Rios Chandler Pentium machine
...
\ **4.2.14.** When installing on a Dell Poweredge XE, Dell proprietary
...
\ **4.2.15.** I have an IBM EtherJet PCI card, it is detected by the
