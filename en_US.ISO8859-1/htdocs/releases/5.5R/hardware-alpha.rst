========================================
FreeBSD/alpha 5.5-RELEASE Hardware Notes
========================================
...
2 `Supported processors and motherboards <#SUPPORT-PROC>`__
2.1 `Overview <#AEN30>`__
2.2 `In general, what do you need to run FreeBSD on an
Alpha? <#AEN46>`__
2.3 `System-specific information <#AEN107>`__
2.3.1 `AXPpci33 (\`\`NoName'') <#AEN112>`__
2.3.2 `Universal Desktop Box (UDB or \`\`Multia'') <#AEN187>`__
2.3.3 `Personal Workstation (\`\`Miata'') <#AEN276>`__
2.3.4 `Evaluation Board 64 family <#AEN386>`__
2.3.5 `Evaluation Board 164 (\`\`EB164, PC164, PC164LX, PC164SX'')
family <#AEN441>`__
2.3.6 `AlphaStation 200 (\`\`Mustang'') and 400 (\`\`Avanti'')
series <#AEN510>`__
2.3.7 `AlphaStation 500 and 600 (\`\`Alcor'' & \`\`Maverick'' for EV5,
\`\`Bret'' for EV56) <#AEN577>`__
2.3.8 `AlphaServer 1000 (\`\`Mikasa''), 1000A (\`\`Noritake'') and
800(\`\`Corelle'') <#AEN689>`__
2.3.9 `DS10/VS10/XP900 (\`\`Webbrick'') / XP1000 (\`\`Monet'') / DS10L
(\`\`Slate'') <#AEN745>`__
2.3.10 `DS20/DS20E (\`\`Goldrush'') <#AEN896>`__
2.3.11 `AlphaPC 264DP / UP2000 <#AEN965>`__
2.3.12 `AlphaServer 2000 (\`\`DemiSable''), 2100 (\`\`Sable''), 2100A
(\`\`Lynx'') <#AEN1011>`__
2.3.13 `AlphaServer 4x00 (\`\`Rawhide'') <#AEN1082>`__
2.3.14 `AlphaServer 1200 (\`\`Tincup'') and AlphaStation 1200
(\`\`DaVinci'') <#AEN1115>`__
2.3.15 `AlphaServer 8200 and 8400 (\`\`TurboLaser'') <#AEN1145>`__
2.3.16 `Alpha Processor Inc. UP1000 <#AEN1188>`__
2.3.17 `Alpha Processor Inc. UP1100 <#AEN1237>`__
2.3.18 `Alpha Processor Inc. CS20, Compaq DS20L <#AEN1285>`__
2.3.19 `Compaq AlphaServer ES40 (\`\`Clipper'') <#AEN1332>`__
2.4 `Supported Hardware Overview <#AEN1369>`__
2.5 `Acknowledgments <#AEN1401>`__
...
5.5-RELEASE on the Alpha/AXP hardware platform (also referred to as
FreeBSD/alpha 5.5-RELEASE). It lists devices known to work on this
...
.. raw:: html
    ...
    **Note:** This document includes information specific to the
    Alpha/AXP hardware platform. Versions of the hardware compatibility
...
In case you have problems making your selection of SRM 'stick', in other
words if the system falls back to AlphaBIOS after you selected the SRM
console, it is time to check if the battery of your CMOS clock/parameter
RAM is empty. If so, a replacement battery most likely fixes the issue.
...
more are recommended. Although Alpha machines typically can accommodate
...
`pcm(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pcm&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The sound device lives at port 0x530, and uses irq 9 along with drq 3.
You also need to specify flags 0x15 in the ``device.hints`` file.
...
on the PC164. Symbios 53C896 has been confirmed to not work.
...
On PC164 the SRM sometimes seems to lose its variable settings. \`\`For
...
The sound device uses port 0x530, IRQ 10 and drq 0. You also need to
specify flags 0x10011 in the ``device.hints`` file.
...
Corresponding entries along with flags 0x10011 must go into the
``device.hints`` file. Note that the flags value is rather non-standard.
...
.. raw:: html
    ...
    **Note:** Contrary to expectation there is no ``cpu EV6`` defined
    for inclusion in the kernel config file. The ``cpu EV5`` is
    mandatory to keep
    `config(8) <http://www.FreeBSD.org/cgi/man.cgi?query=config&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
    happy.
...
but when FreeBSD boots it honors the ``CONSOLE`` variable setting and
all the boot messages as well as the login prompt will go to the serial
port.
...
and a Symbios 810 SCSI chip. VGA console cards must be installed in a
...
various documentation I consulted used both acronyms interchangeably.
...
keyboard/mouse etc. PCI1 has 4 64-bit PCI slots and a Symbios 810 SCSI
...
Contrary to expectation there is no ``cpu EV6`` defined for inclusion in
the kernel config file. The ``cpu EV5`` is mandatory to keep
`config(8) <http://www.FreeBSD.org/cgi/man.cgi?query=config&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
happy.
...
`ata(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ata&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
`de(4) <http://www.FreeBSD.org/cgi/man.cgi?query=de&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
(older driver) or
`dc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=dc&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
(newer driver). Some new SRM versions are known to recognize the Intel
8255x Ethernet chips as driven by the FreeBSD
`fxp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fxp&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver. But beware: the
`fxp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fxp&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver is reported not to work correctly with FreeBSD/alpha (although it
...
1.227.2.35 2006/03/12 23:43:59 rik Exp $
...
The
`ahc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ahc&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
`amr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=amr&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver include:
...
Controllers supported by the
`mlx(4) <http://www.FreeBSD.org/cgi/man.cgi?query=mlx&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver include:
...
The
`ncr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ncr&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`sym(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sym&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
`isp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=isp&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver include:
...
`mpt(4) <http://www.FreeBSD.org/cgi/man.cgi?query=mpt&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver:
...
`cd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=cd&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__).
...
-  ATAPI IDE interface
   (`acd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=acd&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__)
...
`sf(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sf&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver include:
...
The
`pcn(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pcn&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
Adapters supported by the
`rl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=rl&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver include:
...
The
`wb(4) <http://www.FreeBSD.org/cgi/man.cgi?query=wb&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`vr(4) <http://www.FreeBSD.org/cgi/man.cgi?query=vr&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`sis(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sis&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`nge(4) <http://www.FreeBSD.org/cgi/man.cgi?query=nge&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`ste(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ste&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
Adapters supported by the
`sk(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sk&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver include:
...
The
`tl(4) <http://www.FreeBSD.org/cgi/man.cgi?query=tl&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`txp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=txp&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`em(4) <http://www.FreeBSD.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`gx(4) <http://www.FreeBSD.org/cgi/man.cgi?query=gx&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`hme(4) <http://www.FreeBSD.org/cgi/man.cgi?query=hme&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`re(4) <http://www.FreeBSD.org/cgi/man.cgi?query=re&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
DEC DEFPA PCI
(`fpa(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fpa&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)
...
(`sio(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sio&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)

AST 4 port serial card using shared IRQ

Comtrol Rocketport card
(`rp(4) <http://www.FreeBSD.org/cgi/man.cgi?query=rp&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)
...
The
`sound(4) <http://www.FreeBSD.org/cgi/man.cgi?query=sound&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`snd\_maestro(4) <http://www.FreeBSD.org/cgi/man.cgi?query=snd_maestro&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`snd\_maestro3(4) <http://www.FreeBSD.org/cgi/man.cgi?query=snd_maestro3&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`snd\_sbc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=snd_sbc&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver supports the following sound cards:
...
The
`ohci(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ohci&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`uhci(4) <http://www.FreeBSD.org/cgi/man.cgi?query=uhci&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
Keyboards
(`ukbd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ukbd&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)

Mice
(`ums(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ums&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)

The
`ulpt(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ulpt&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`umct(4) <http://www.FreeBSD.org/cgi/man.cgi?query=umct&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
The
`umass(4) <http://www.FreeBSD.org/cgi/man.cgi?query=umass&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
...
Floppy drives
(`fdc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=fdc&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)

VGA-compatible video cards
(`vga(4) <http://www.FreeBSD.org/cgi/man.cgi?query=vga&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)
...
-  AT-style keyboards
   (`atkbd(4) <http://www.FreeBSD.org/cgi/man.cgi?query=atkbd&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
   driver)
...
-  PS/2 mice and compatible devices, including many laptop pointing
   devices
   (`psm(4) <http://www.FreeBSD.org/cgi/man.cgi?query=psm&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
   driver)
...
.. raw:: html
    ...
    **Note:**
    `moused(8) <http://www.FreeBSD.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+5.5-RELEASE>`__
...
(`ppc(4) <http://www.FreeBSD.org/cgi/man.cgi?query=ppc&sektion=4&manpath=FreeBSD+5.5-RELEASE>`__
driver)
