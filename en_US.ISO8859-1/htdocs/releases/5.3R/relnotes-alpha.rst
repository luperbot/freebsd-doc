...
The ``tga`` driver has been removed from GENERIC as it needs major work
to stop it from causing panics on kernel startup. The use of a serial
console or a plain ``vga`` card is suggested as an alternative.
