...
On FreeBSD/sparc64, time\_t has been changed from a 32-bit value to a
64-bit value.
...
.. raw:: html
    ...
    **Note:** Since this change is not backward-compatible, any programs
    which were built on an older system using a 32-bit time\_t and call
    system routines for handling time\_t values, will have to be
    recompiled. More detailed information and notice on upgrading from
    the source can be found in ``/usr/src/UPDATING.64BTT``.
...
driver now supports the BREAK\_TO\_DEBUGGER kernel option.
...
driver which prevents the cards on SBus from working correctly, has been
fixed.
...
device driver has been ported from NetBSD to support the SBus SCSI card
in Sun Ultra 1e and 2 machines.
...
now supports two new flags: -c to calculate all partition sizes in
cylinders as opposed to sectors, and -h to print the label in human
readable size/offset format.
