...
been added (or changed) since 4.8-RELEASE. It also provides some notes
...
The
`safe(4) <http://www.FreeBSD.org/cgi/man.cgi?query=safe&sektion=4&manpath=FreeBSD+4.9-RELEASE>`__
...
A bug which prevented the kernel from booting on an Intel 80386
...
any single process (or the FreeBSD kernel) is unchanged. The
`pae(4) <http://www.FreeBSD.org/cgi/man.cgi?query=pae&sektion=4&manpath=FreeBSD+4.9-RELEASE>`__
manual page has more details on this feature.
...
driver for the Granch SBNI16 SHDSL modem has been added.
...
The Intel ACPI 20030228 distribution (with local FreeBSD changes and
support code) has been imported. This feature should be considered
experimental and should be tested prior to being deployed in a
...
.. raw:: html
    ...
    **Note:** Unlike on FreeBSD-CURRENT, the ACPI driver must be
    statically compiled into the kernel by adding device acpica to a
    kernel configuration. There is no kernel module. This driver is not
    present in the default, ``GENERIC`` kernel.
